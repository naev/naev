...
static LuaUniform_t *shader_getUniform( const LuaShader_t *ls,
                                        const char        *name )
{
   return bsearch( name, ls->uniforms, ls->nuniforms, sizeof( LuaUniform_t ),
...
static int shaderL_new( lua_State *L )
{
   ...
   /* Do from string. */
   shader.program = gl_program_vert_frag_string(
      vertexcode, strlen( vertexcode ), pixelcode, strlen( pixelcode ) );
   if ( shader.program == 0 )
...
#undef UNIFORM
   ...
   for ( GLint i = 0; i < shader.nuniforms; i++ ) {
      ...
      glGetActiveUniform( shader.program, (GLuint)i, SHADER_NAME_MAXLEN,
                          &length, &u->size, &u->type, u->name );
      u->id  = glGetUniformLocation( shader.program, u->name );
      u->tex = -1;

      /* Textures need special care. */
      if ( ( u->type == GL_SAMPLER_2D ) &&
           ( strcmp( u->name, "MainTex" ) != 0 ) ) {
         if ( shader.tex == NULL )
 ...
 */
void shader_parseUniformArgsFloat( GLfloat values[4], lua_State *L, int idx,
                                   int n )
{
...
static int shaderL_addPostProcess( lua_State *L )
{
   ...
   if ( strcmp( str, "final" ) == 0 )
      ...
      return NLUA_ERROR( L,
                         _( "Layer was '%s', but must be one of 'final', "
                            "'game', 'gui', or 'core'." ),
                         str );
