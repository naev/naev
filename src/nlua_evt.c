...
static const luaL_Reg evtL_methods[] = {
   ...
   { "claim", evtL_claim },   { 0, 0 } }; /**< Mission Lua methods. */
...
int event_runFunc( unsigned int eventid, const char *func, int nargs )
{
   ...
   ev =
      event_get( eventid ); /* The Lua call may have invalidated the pointer. */
   if ( ev == NULL ) {
      WARN( _( "Event deleted in middle of hook call! Likely caused by another "
               "hook finishing the event triggered by this hook (\"%s\")." ),
            func );
      return -1;
   }
 ...
 * @brief Adds an NPC.
 *
 * @usage npc_id = evt.npcAdd( "my_func", "Mr. Test", "none.webp", "A test." )
 * -- Creates an NPC.
 *
 ...
 *    @luatparam[opt=event.priority] number priority Optional priority argument
...
static int evtL_npcAdd( lua_State *L )
{
   ...
   /* Optional parameters. */
   priority = luaL_optinteger( L, 5, event_priority( cur_event->id ) );
   ...
   /* Add npc. */
   id =
      npc_add_event( cur_event->id, func, name, priority, portrait, desc, bg );
 ...
 *
 *    @luatparam[opt=false] boolean properly If true and the event is unique it
 * marks the event as completed. If false it deletes the event but doesn't mark
 * it as completed.
 * @luafunc finish
 */
static int evtL_finish( lua_State *L )
 ...
 *
 *    @luatparam[opt=true] boolean enable If true sets the event to save,
 * otherwise tells the event to not save.
 * @luafunc save
 */
static int evtL_save( lua_State *L )
{
 ...
 *
 * @usage if not evt.claim( { system.get("Gamma Polaris") } ) then evt.finish(
 * false ) end
 * @usage if not evt.claim( system.get("Gamma Polaris") ) then evt.finish( false
 * ) end
 * @usage if not evt.claim( 'some_string' ) then evt.finish( false ) end
 * @usage if not evt.claim( { system.get("Gamma Polaris"), 'some_string' } )
