...
/* dialogues */
static glFont *dialogue_getSize( const char *title, const char *msg, int *width,
                                 int *height );
...
typedef struct InputDialogue_ {
   ...
   void ( *item_select_cb )( unsigned int wid, const char *wgtname, int x,
                             int y, int w,
                             int h ); /**< callback when an item is selected */
} InputDialogue;
...
void dialogue_alertRaw( const char *msg )
{
   ...
   msg_wid = window_create( "dlgAlert", caption, -1, -1, w, 110 + h );
   window_setData( msg_wid, &done );
   window_addText( msg_wid, 20, -40, w - 40, h, 0, "txtMsg", font, NULL, msg );
                         ...
                         int width, int height )
{
   ...
   /* IMPORTANT : texture must not be freed here, it will be freed when the
    * widget closes */
   ...
   /* Create the window */
   msg_wid =
      window_create( "dlgMsgImg", caption, -1, -1, img_width + w, 110 + h );
   window_setData( msg_wid, &done );

   /* Add the text box */
   window_addText( msg_wid, img_width + 40, -40, w - 40, h, 0, "txtMsg", font,
                   NULL, msg );

   /* Add a placeholder rectangle for the image */
   window_addRect( msg_wid, 20, -40, img_width, img_height, "rctGFX", &cGrey10,
                   1 );

   /* Actually add the texture in the rectangle */
   window_addImage( msg_wid, 20, -40, img_width, img_height, "ImgGFX", gfx, 0 );

   /* Add the OK button */
   window_addButton( msg_wid, ( img_width + w - 50 ) / 2, 20, 50, 30, "btnOK",
                     _( "OK" ), dialogue_close );
...
static InputDialogue
   input_dialogue; /**< Stores the input window id and callback. */
/**
...
char *dialogue_inputRaw( const char *title, int min, int max, const char *msg )
{
   ...
   input_dialogue.input_wid =
      window_create( "dlgInput", title, -1, -1, w + 40, h + 140 );
   ...
   /* text */
   window_addText( input_dialogue.input_wid, 20, -30, w, h, 0, "txtInput", font,
                   NULL, msg );
   /* input */
   window_addInput( input_dialogue.input_wid, 20, 20 + 30 + 10, w, 30,
                    "inpInput", max, 1, NULL );
   ...
   while ( ( done >= 0 ) &&
      ...
      if ( input ) {
         dialogue_alert( n_( "Input must be at least %d character long!",
                             "Input must be at least %d characters long!",
                             min ),
                         min );
         free( input );
         input = NULL;
      ...
      if ( done < 0 )
         ...
         input =
            strdup( window_getInput( input_dialogue.input_wid, "inpInput" ) );
   }
...
/**
 * @brief Creates a list dialogue with OK and Cancel button with a fixed
 * message.
 *
 *    @param title Title of the dialogue.
...
/**
 * @brief Creates a list dialogue with OK and Cancel buttons, with a fixed
 * message, as well as a small extra area for the list to react to item selected
 * events.
 *
 *    @param title Title of the dialogue.
 *    @param items Items in the list (should be all malloced, automatically
 ...
 *    @param add_widgets This function is called with the new window as an
 * argument allowing for initial population of the extra area.
 ...
 *    @param add_widgets This function is called with the new window as an
 * argument allowing for initial population of the extra area.
 *    @param select_call (optional) This function is called when a new item in
 * the list is selected, receiving the window's id and the selected widgets name
 * as arguments.
 *    @param msg string with text to display.
 */
   ...
   const char *msg )
{
   ...
   window_setData( wid, &done );
   window_addText( wid, 20, -40, w - 40, text_height, 0, "txtMsg", font, NULL,
                   msg );
   ...
   /* Create the list. */
   window_addList( wid, 20, -40 - text_height - 20, w - 40,
                   h - ( 40 + text_height + 20 ) - ( 20 + 30 + 20 ),
                   "lstDialogue", items, nitems, 0, select_call_wrapper,
                   dialogue_listClose );

   /* Create the buttons. */
   window_addButton( wid, -20, 20, 120, 30, "btnOK", _( "OK" ),
                     dialogue_listClose );
   window_addButton( wid, -20 - 120 - 20, 20, 120, 30, "btnCancel",
                     _( "Cancel" ), dialogue_listCancel );
...
void dialogue_makeChoice( const char *caption, const char *msg, int opts )
{
   ...
   /* create window */
   choice_wid = window_create( "dlgChoice", caption, -1, -1, w,
                               h + 100 + 40 * choice_nopts );
   /* text */
   window_addText( choice_wid, 20, -40, w - 40, h, 0, "txtChoice", font, NULL,
                   msg );
}
/**
...
void dialogue_addChoice( const char *caption, const char *msg, const char *opt )
{
   ...
   window_addButton( choice_wid, w / 2 - 125, choice_nopts * 40, 250, 30,
                     (char *)opt, (char *)opt, dialogue_choiceClose );
   choice_nopts--;
}
/**
                      ...
                      void *data, int autofree, int dynamic )
{
   ...
   window_addCust( wid, wgtx, wgty, width, height, "cstCustom", 0, render, NULL,
                   NULL, NULL, data );
   if ( dynamic )
 ...
 * basically we have a race condition due to the "threading" effect this
 * creates. Solved most of it by removing globals in the Lua event/mission code,
 * but this doesn't mean it's solved. It just means it's extremely unlikely.
 *
 *    @return 0 on success.
 */
static int toolkit_loop( int *loop_done, dialogue_update_t *du )
