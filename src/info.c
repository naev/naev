...
#define menu_Close( f )                                                        \
   ( menu_open &= ~( f ) ) /**< Marks a menu as closed.                        \
                            */
...
static const char *info_names[INFO_WINDOWS] = {
   N_( "Main" ),     N_( "Ship" ),      N_( "Weapons" ),  N_( "Cargo" ),
   N_( "Missions" ), N_( "Standings" ), N_( "Ship log" ),
...
static void        info_changeTab( unsigned int wid, const char *str, int old,
                                   int new );
...
static void info_buttonRegen( void )
{
   ...
   for ( int i = 0; i < array_size( info_buttons ); i++ ) {
      ...
      window_addButtonKey( wid, -20 - c * ( 20 + BUTTON_WIDTH ),
                           20 + r * ( 20 + BUTTON_HEIGHT ), BUTTON_WIDTH,
                           BUTTON_HEIGHT, btn->button, btn->caption,
                           info_buttonClick, btn->key );
   }
   window_resizeWidget( wid, "lstInventory", w - 80 - 300 - 40 - 40,
                        h - 90 - rows * ( 20 + BUTTON_HEIGHT ) );
 ...
 *    @param key Hotkey for using the button without it being focused (or
 * SDLK_UNKNOWN or 0 if none).
...
int info_buttonRegister( const char *caption, int priority, SDL_Keycode key )
{
   ...
   id = btn->id;
   qsort( info_buttons, array_size( info_buttons ), sizeof( InfoButton_t ),
          sort_buttons );
 ...
 *
 *    @param id ID of the button to unregister and previously created by
 * info_buttonRegister.
 *    @return 0 on success.
 */
...
static void info_openMain( unsigned int wid )
{
   ...
   k += scnprintf( &str[k], sizeof( str ) - k, "\n%s", _( "Fuel:" ) );
   k += scnprintf( &str[k], sizeof( str ) - k, "\n%s",
                   ( player.fleet_capacity > 0 ) ? _( "Cargo (fleet):" )
                                                 : _( "Cargo:" ) );
   k += scnprintf( &str[k], sizeof( str ) - k, "\n\n%s", _( "Time played:" ) );
   k += scnprintf( &str[k], sizeof( str ) - k, "\n%s", _( "Times died:" ) );
   k += scnprintf( &str[k], sizeof( str ) - k, "\n%s", _( "Times jumped:" ) );
   ...
   /*k +=*/scnprintf( &str[k], sizeof( str ) - k, "\n%s",
                      _( "Ships destroyed:" ) );
   window_addText( wid, 20, 20, 160, h - 80, 0, "txtDPilot", &gl_smallFont,
                   &cFontGrey, str );
   ...
   l += scnprintf( &str[l], sizeof( str ) - l, "\n%s", player.p->name );
   l += scnprintf( &str[l], sizeof( str ) - l, "\n%.0f %s (%d %s)",
                   player.p->fuel, UNIT_UNIT, pilot_getJumps( player.p ),
   ...
   l += scnprintf( &str[l], sizeof( str ) - l, "\n%s",
                   num2strU( (double)player.jumped_times, 0 ) );
   l += scnprintf( &str[l], sizeof( str ) - l, "\n%s\n",
                   num2strU( (double)player.landed_times, 0 ) );
   l +=
      scnprintf( &str[l], sizeof( str ) - l, _( "%s %s" ),
                 num2strU( player.dmg_done_shield + player.dmg_done_armour, 0 ),
                 UNIT_ENERGY );
   l += scnprintf( &str[l], sizeof( str ) - l, "\n" );
   l += scnprintf(
      &str[l], sizeof( str ) - l, _( "%s %s" ),
      num2strU( player.dmg_taken_shield + player.dmg_taken_armour, 0 ),
      UNIT_ENERGY );
   /*l +=*/scnprintf( &str[l], sizeof( str ) - l, "\n%s",
                      num2strU( destroyed, 0 ) );
   window_addText( wid, 200, 20, w - 80 - 200 - 40 + 20 - 180, h - 80, 0,
                   "txtPilot", &gl_smallFont, NULL, str );
   free( nt );
   ...
   if ( n == 0 ) {
      ...
      for ( int i = 0; i < ninv; i++ ) {
         ...
         if ( pi->quantity == 0 )
            ...
            SDL_asprintf( &inventory[nlic + i], _( "%s (%s)" ), _( pi->name ),
                          num2strU( pi->quantity, 0 ) );
      }
      qsort( &inventory[nlic], ninv, sizeof( char * ), strsort );
   }
   window_addText( wid, -20, -40, w - 80 - 300 - 40 - 40, 20, 1, "txtList",
                   NULL, NULL, _( "Inventory" ) );
   window_addList( wid, -20, -70, w - 80 - 300 - 40 - 40,
                   h - 110 - BUTTON_HEIGHT, "lstInventory", inventory, n, 0,
                   NULL, NULL );
...
static void info_openShip( unsigned int wid )
{
   ...
   l += scnprintf( &buf[l], sizeof( buf ) - l, "\n" );
   /*l +=*/scnprintf( &buf[l], sizeof( buf ) - l, "\n%s", _( "Stats:" ) );
   window_addText( wid, 20, -40, 160, h - 60, 0, "txtSDesc", &gl_smallFont,
                   &cFontGrey, buf );
   window_addText( wid, 200, -40, w - 20 - 20 - 20 - 200 - 180., h - 60, 0,
                   "txtDDesc", &gl_smallFont, NULL, NULL );
...
static void ship_update( unsigned int wid )
{
   ...
   l += scnprintf( &buf[l], sizeof( buf ) - l, _( "%s %s" ),
                   num2strU( player.p->solid.mass, 0 ), UNIT_MASS );
   l += scnprintf( &buf[l], sizeof( buf ) - l, "\n" );
   l += scnprintf( &buf[l], sizeof( buf ) - l, _( "%s average" ), hyp_delay );
   l += scnprintf( &buf[l], sizeof( buf ) - l, "\n" );
   l += scnprintf( &buf[l], sizeof( buf ) - l, _( "%.0f %s" ), player.p->accel,
                   UNIT_ACCEL );
   l += scnprintf( &buf[l], sizeof( buf ) - l, "\n" );
   l += scnprintf(
      &buf[l], sizeof( buf ) - l, _( "%.0f %s (max %.0f %s)" ), player.p->speed,
      UNIT_SPEED,
      solid_maxspeed( &player.p->solid, player.p->speed, player.p->accel ),
      UNIT_SPEED );
   l += scnprintf( &buf[l], sizeof( buf ) - l, "\n" );
   l += scnprintf( &buf[l], sizeof( buf ) - l, _( "%.0f %s" ),
                   player.p->turn * 180. / M_PI, UNIT_ROTATION );
   l +=
   ...
   l += scnprintf( &buf[l], sizeof( buf ) - l, "\n" );
   l += scnprintf( &buf[l], sizeof( buf ) - l, _( "%s / %s %s" ), sarmour,
                   num2strU( player.p->armour_max, 0 ), UNIT_ENERGY );
   l += scnprintf( &buf[l], sizeof( buf ) - l, _( " (%s %s)" ),
                   num2strU( player.p->armour_regen, 1 ), UNIT_POWER );
   l += scnprintf( &buf[l], sizeof( buf ) - l, "\n" );
   l += scnprintf( &buf[l], sizeof( buf ) - l, _( "%s / %s %s" ), senergy,
                   num2strU( player.p->energy_max, 0 ), UNIT_ENERGY );
   l += scnprintf( &buf[l], sizeof( buf ) - l, _( " (%s %s)" ),
                   num2strU( player.p->energy_regen, 1 ), UNIT_POWER );
   l += scnprintf( &buf[l], sizeof( buf ) - l, "\n" );
   l += scnprintf( &buf[l], sizeof( buf ) - l, _( "%s / %s %s" ), scargo,
                   num2strU( cargo, 0 ), UNIT_MASS );
   l += scnprintf( &buf[l], sizeof( buf ) - l, "\n" );
   l += scnprintf( &buf[l], sizeof( buf ) - l, _( "%s / %s %s (%d %s)" ), sfuel,
                   num2strU( player.p->fuel_max, 0 ), UNIT_UNIT,
                   pilot_getJumps( player.p ),
                   n_( "jump", "jumps", pilot_getJumps( player.p ) ) );
...
static void info_openWeapons( unsigned int wid )
{
   ...
   y -= 28;
   window_addCheckbox(
      wid, x + 10, y, wlen, BUTTON_HEIGHT, "chkVolley",
      _( "Enable volley mode (weapons fire jointly)" ), weapons_volley,
      pilot_weapSetVolleyCheck( player.p, info_eq_weaps.weapons ) );
   y -= 28;
   window_addCheckbox(
      wid, x + 10, y, wlen, BUTTON_HEIGHT, "chkInrange",
      _( "Only shoot weapons that are in range" ), weapons_inrange,
      pilot_weapSetInrangeCheck( player.p, info_eq_weaps.weapons ) );
   y -= 28;
   window_addCheckbox( wid, x + 10, y, wlen, BUTTON_HEIGHT, "chkHelper",
...
{
   (void)str;
   int sure = dialogue_YesNoRaw(
      _( "Clear All Weapon Sets?" ),
...
static void info_openCargo( unsigned int wid )
{
   ...
   /* Description. */
   window_addText( wid, 20, -40 - 200 - 20, w - 40, h - BUTTON_HEIGHT - 260, 0,
...
static void cargo_genList( unsigned int wid )
{
   ...
   array_free( pclist );
   window_addList( wid, 20, -40, w - 40, 200, "lstCargo", buf, nbuf, 0,
                   cargo_update, NULL );
...
static void info_openStandings( unsigned int wid )
{
   ...
   /* Display list. */
   window_addList( wid, 20, -40, lw, h - 60, "lstStandings", str,
...
static void standings_update( unsigned int wid, const char *str )
{
   ...
   l += scnprintf(
      &buf[l], sizeof( buf ) - l,
      _( "You can have a maximum reputation of %.0f%% with this faction." ),
      round( faction_reputationMax( info_factions[p] ) ) );
...
static void info_openMissions( unsigned int wid )
{
   ...
   window_addCheckbox(
      wid, 300 + 40, 20 + BUTTON_HEIGHT + 10 + gl_defFont.h + 10, w - 300 - 60,
      BUTTON_HEIGHT, "chkPrefer", _( "Prioritize the mission" ),
      mission_menu_chk_priority, 0 );

   /* Mission Description */
   window_addText( wid, 300 + 40, -40, w - 300 - 60, h - BUTTON_HEIGHT - 120, 0,
...
static void mission_menu_genList( unsigned int wid, int first )
{
   ...
   /* list */
   misn_names =
      malloc( sizeof( char * ) * MAX( 1, array_size( player_missions ) ) );
   selectedMission = -1;
   j               = 0;
   for ( int i = 0; i < array_size( player_missions ); i++ )
      if ( player_missions[i]->id != 0 )
         misn_names[j++] = ( player_missions[i]->title != NULL )
                              ? strdup( player_missions[i]->title )
                              : NULL;
   ...
   window_addList( wid, 20, -40, 300, h - 340, "lstMission", misn_names, j,
                   selectedMission, mission_menu_update, NULL );
...
/* amount of screen available for logs: -20 below button, -20 above button, -40
 * from top, -20 x2 between logs.*/
...
static void shiplog_menu_update( unsigned int wid, const char *str )
{
   ...
   if ( strcmp( str, "lstLogEntries" ) != 0 ) {
      ...
      if ( logType != selectedLogType ) {
         ...
         logs = NULL;
         shiplog_listLogsOfType( info_getLogTypeFilter( selectedLogType ),
                                 &nlogs, &logs, &logIDs, 1 );
         if ( selectedLog >= nlogs )
            selectedLog = 0;
         window_addList( wid, 20, 60 + BUTTON_HEIGHT + LOGSPACING / 2, w - 40,
                         LOGSPACING / 4, "lstLogs", logs, nlogs, 0,
                         shiplog_menu_update, NULL );
      ...
      if ( regenerateEntries || selectedLog != log ) {
         ...
         shiplog_listLog( logIDs[selectedLog],
                          info_getLogTypeFilter( selectedLogType ), &nentries,
                          &logentries, 1 );
         window_addList( wid, 20, 40 + BUTTON_HEIGHT, w - 40,
                         LOGSPACING / 2 - 20, "lstLogEntries", logentries,
                         nentries, 0, shiplog_menu_update, info_shiplogView );
...
static void shiplog_menu_genList( unsigned int wid, int first )
{
   ...
   shiplog_listLogsOfType( info_getLogTypeFilter( selectedLogType ), &nlogs,
                           &logs, &logIDs, 1 );
   if ( selectedLog >= nlogs )
      selectedLog = 0;
   ...
   shiplog_listLog( logIDs[selectedLog],
                    ...
                    &logentries, 1 );
   logWidgetsReady = 0;
   window_addList( wid, 20, 80 + BUTTON_HEIGHT + 3 * LOGSPACING / 4, w - 40,
                   LOGSPACING / 4, "lstLogType", logTypes, ntypes, 0,
                   shiplog_menu_update, NULL );
   window_addList( wid, 20, 60 + BUTTON_HEIGHT + LOGSPACING / 2, w - 40,
                   LOGSPACING / 4, "lstLogs", logs, nlogs, 0,
                   shiplog_menu_update, NULL );
   window_addList( wid, 20, 40 + BUTTON_HEIGHT, w - 40, LOGSPACING / 2 - 20,
                   "lstLogEntries", logentries, nentries, 0,
...
static void info_shiplogMenuDelete( unsigned int wid, const char *str )
{
   ...
   if ( logIDs[selectedLog] == LOG_ID_ALL ) {
      dialogue_msg( "", _( "You are currently viewing all logs in the selected "
                           "log type. Please select a log title to delete." ) );
      return;
   }

   snprintf( buf, sizeof( buf ),
             _( "This will delete ALL \"%s\" log entries. This operation "
                "cannot be undone. Are you sure?" ),
             logs[selectedLog] );
   ret = dialogue_YesNoRaw( "", buf );
   if ( !ret )
      return;
   /* There could be several logs of the same name, so make sure we get the
    * correct one. */
   /* selectedLog-1 since not including the "All" */
   logid = shiplog_getIdOfLogOfType( info_getLogTypeFilter( selectedLogType ),
                                     selectedLog - 1 );
   if ( logid >= 0 )
...
static void info_shiplogAdd( unsigned int wid, const char *str )
{
   ...
   if ( log < 0 || logIDs[log] == LOG_ID_ALL ) {
      tmp = dialogue_inputRaw( _( "Add a log entry" ), 0, 4096,
                               _( "Add an entry to your diary:" ) );
      ...
      tmp = dialogue_input( _( "Add a log entry" ), 0, 4096,
                            _( "Add an entry to the log titled '%s':" ),
                            logs[log] );
      if ( ( tmp != NULL ) && ( strlen( tmp ) > 0 ) ) {
         int logid = shiplog_getIdOfLogOfType( info_getLogTypeFilter( logType ),
                                               log - 1 );
         if ( logid >= 0 )
            ...
            dialogue_msgRaw(
               _( "Cannot add log" ),
               _( "Cannot find this log!  Something went wrong here!" ) );
         free( tmp );
      }
   }
   shiplog_menu_genList( wid, 0 );
...
static void info_openShipLog( unsigned int wid )
{
   ...
   window_addText( wid, 20, 60 + BUTTON_HEIGHT + 3 * LOGSPACING / 4, w - 40,
                   texth, 0, "logDesc2", &gl_smallFont, NULL,
                   _( "Select log title:" ) );

   window_addText( wid, 20, 25 + BUTTON_HEIGHT + LOGSPACING / 2, w - 40, texth,
                   0, "logDesc3", &gl_smallFont, NULL, _( "Log entries:" ) );
                            ...
                            int new )
{
   ...
   case INFO_WIN_SHIP:
      ...
      break;
   case INFO_WIN_WEAP:
      hookname = "info_weapons";
      break;
   case INFO_WIN_CARGO:
      hookname = "info_cargo";
      break;
   case INFO_WIN_MISN:
      hookname = "info_mission";
      break;
   case INFO_WIN_STAND:
      hookname = "info_standing";
      break;
   case INFO_WIN_SHIPLOG:
