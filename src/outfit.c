...
double outfit_range( const Outfit* o )
{
   if (outfit_isBolt(o))
      ...
      if (o->u.amm.thrust) {
         double speedinc = o->u.amm.speed_max - o->u.amm.speed;
         double at = speedinc / o->u.amm.thrust;
         if (at < o->u.amm.duration)
            return speedinc * (o->u.amm.duration - at / 2.) + o->u.amm.speed * o->u.amm.duration;

         /* Maximum speed will never be reached. */
         return pow2(o->u.amm.duration) * o->u.amm.thrust / 2. + o->u.amm.duration * o->u.amm.speed;
      }
...
double outfit_speed( const Outfit* o )
{
   if (outfit_isBolt(o))
      ...
      if (o->u.amm.thrust == 0.)
         ...
         double t = (o->u.amm.speed_max - o->u.amm.speed) / o->u.amm.thrust; /* Time to reach max speed */
         /* Reaches max speed. */
...
static void outfit_parseSBeam( Outfit* temp, const xmlNodePtr parent )
{
   ...
   temp->u.bem.lua_onshoot    = LUA_NOREF;
...
int outfit_load (void)
{
   ...
   for (int i=0; i<noutfits; i++) {
      ...
      if (outfit_isLauncher(o)) {
         ...
         nlua_env env;
         size_t sz;
         char *dat = ndata_read( o->u.bem.lua_file, &sz );
         if (dat==NULL) {
            WARN(_("Outfit '%s' failed to read Lua '%s'!"), o->name, o->u.bem.lua_file );
            continue;
         }
         ...
         /* Run code. */
         if (nlua_dobufenv( env, dat, sz, o->u.bem.lua_file ) != 0) {
            WARN(_("Outfit '%s' Lua error:\n%s"), o->name, lua_tostring(naevL,-1));
            lua_pop(naevL,1);
            nlua_freeEnv( o->u.bem.lua_env );
            free( dat );
            o->u.bem.lua_env = LUA_NOREF;
            continue;
         }
         free( dat );
         ...
         o->u.bem.lua_onshoot    = nlua_refenvtype( env, "onshoot",  LUA_TFUNCTION );
...
void outfit_free (void)
{
   for (int i=0; i < array_size(outfit_stack); i++) {
      ...
      /* Type specific. */
         ...
         free(o->u.blt.lua_file);
      }
      else if (outfit_isBolt(o))
         free(o->u.bem.lua_file);
      else if (outfit_isLauncher(o))
