...
#define HYPERSPACE_DUST_LENGTH                                                 \
   250 /**< Length the space dust blur to at max (pixels). */
#define HYPERSPACE_FADEOUT 1. /**< How long the fade is (seconds). */
#define HYPERSPACE_FADEIN 1.  /**< How long the fade is (seconds). */
#define HYPERSPACE_ACCEL                                                       \
   2000. /**< How much pilots accelerate when jumping.                         \
          */
#define HYPERSPACE_VEL                                                         \
   ( 2. * HYPERSPACE_ACCEL *                                                   \
     HYPERSPACE_FLY_DELAY ) /**< Velocity at hyperspace. */
#define HYPERSPACE_ENTER_MIN                                                   \
   ( HYPERSPACE_VEL * 0.3 ) /**< Minimum entering distance. */
#define HYPERSPACE_ENTER_MAX                                                   \
   ( HYPERSPACE_VEL * 0.4 )       /**< Maximum entering distance. */
...
#define PILOT_PLAYER_NONTARGETABLE_JUMPIN_DELAY                                \
   5. /**< Time the player is safe (from being targetted) after jumping in. */
...
/* Damage */
#define PILOT_HOSTILE_THRESHOLD                                                \
   0.09                           /**< Point at which pilot becomes hostile. */
#define PILOT_HOSTILE_DECAY 0.005 /**< Rate at which hostility decays. */

/* Makes life easier */
#define pilot_isPlayer( p )                                                    \
   pilot_isFlag( p, PILOT_PLAYER ) /**< Checks if pilot is a player. */
#define pilot_isDisabled( p )                                                  \
   pilot_isFlag( p, PILOT_DISABLED ) /**< Checks if pilot is disabled. */
#define pilot_isStopped( p ) ( VMOD( p->solid.vel ) <= MIN_VEL_ERR )
/* We would really have to recursively go up all the parents to check, but we're
 * being cheap. */
#define pilot_isWithPlayer( p )                                                \
   ( ( p )->faction == FACTION_PLAYER || ( ( p )->parent == PLAYER_ID ) )
...
#define PILOTOUTFIT_ACTIVE                                                     \
   ( 1 << 0 ) /**< Pilot outfit is an active outfit.                           \
               */
#define PILOTOUTFIT_TOGGLEABLE                                                 \
   ( 1 << 1 )                       /**< Pilot outfit is an active outfit. */
#define PILOTOUTFIT_ISON ( 1 << 2 ) /**< Pilot outfit should be turned on. */
#define PILOTOUTFIT_VOLLEY                                                     \
   ( 1 << 3 ) /**< Pilot outfit is part of a volley weapon set (and ison). */
#define PILOTOUTFIT_INRANGE                                                    \
   ( 1 << 4 ) /**< Pilot outfit is part of an inrange weapon set (and ison).   \
               */
#define PILOTOUTFIT_MANUAL                                                     \
...
#define PILOTOUTFIT_DYNAMIC_FLAGS                                              \
   ( PILOTOUTFIT_VOLLEY | PILOTOUTFIT_INRANGE | PILOTOUTFIT_MANUAL |           \
     PILOTOUTFIT_ISON_TOGGLE | PILOTOUTFIT_ISON_HOLD )
...
typedef struct PilotOutfitSlot_ {
   ...
   /* Type-specific data. */
   union {
      unsigned int beamid;  /**< ID of the beam used in this outfit, only used
                               for beams. */
   ...
   int
      lua_mem; /**< Lua reference to the memory table of the specific outfit. */
   ShipStatList *lua_stats; /**< Intrinsic ship stats for the outfit calculated
                               on the fly. Used only by Lua outfits. */
} PilotOutfitSlot;
...
typedef struct Pilot_ {
   ...
   double
      ew_detection; /**< Main detection. Affects both signature and stealth.*/
   double ew_signature; /**< Signature. Used for tracking and blurring
                           identification. */
   ...
   ShipStatList *ship_stats; /**< Ship stats set by the Lua ship. Only valid if
                                the Lua has ship defined. */
   ShipStatList *intrinsic_stats; /**< Intrinsic statistics to the ship create
                                     on the fly. */
   ShipStats
      stats; /**< Pilot's copy of ship statistics, used for comparisons.. */
   ...
   PilotOutfitSlot
      *outfit_structure; /**< Array (array.h): The structure slots. */
   ...
   PilotOutfitSlot
      *outfit_intrinsic; /**< Array (array.h): The intrinsic slots. */
   ...
   unsigned int dockpilot; /**< Pilot's dock pilot (the pilot it originates
                           from). This is separate from parent because it needs
                           to be set in sync with dockslot (below). Used to
                           unset dockslot when the dock pilot is destroyed. */
   ...
   unsigned int shoot_indicator; /**< Indicator to inform the AI if a seeker has
                                    been shot recently. */
...
void    pilot_untargetPilot( const Pilot *p );
