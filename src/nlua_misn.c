 ...
 *
 *    @luatparam string|number reward Description of the reward to use. Can pass
 * a number to signify a monetary reward, and allow for sorting.
 * @luafunc setReward
 */
static int misn_setReward( lua_State *L )
{
...
static int misn_markerMove( lua_State *L )
{
   ...
   if ( marker == NULL )
      return NLUA_ERROR( L, _( "Mission does not have a marker with id '%d'" ),
                         id );
 ...
 *
 *    @luatparam[opt] number id ID of the marker to remove. If no parameter is
 * passed, all markers associated with the mission are removed.
 * @luafunc markerRm
 */
static int misn_markerRm( lua_State *L )
{
   ...
   if ( lua_isnoneornil( L, 1 ) ) {
      array_erase( &cur_mission->markers, array_begin( cur_mission->markers ),
                   array_end( cur_mission->markers ) );
      mission_sysMark();
      return 0;
   }
 ...
 *    @luatreturn {Faction,...} A table containing the factions for whom the
 ...
 *
 * Note: there is no limit on the maximum number of missions a player can have
 * simultaneously.
 *
...
static int misn_accept( lua_State *L )
{
   ...
   for ( int i = array_size( player_missions ) - 1; i >= 0; i-- ) {
      ...
      mission_cleanup( m );
      free( m );
      array_erase( &player_missions, &player_missions[i],
                   &player_missions[i + 1] );
   }
 ...
 *
 *    @luatparam[opt] boolean properly If true and the mission is unique it
 * marks the mission as completed.  If false it deletes the mission but doesn't
 * mark it as completed.  If the parameter isn't passed it just ends the mission
 * (without removing it from the player's list of active missions).
 * @luafunc finish
 */
static int misn_finish( lua_State *L )
{
...
/**
 * @brief Adds some mission cargo to the player. They cannot sell it nor get rid
 * of it unless they abandons the mission in which case it'll get eliminated.
 *
 ...
 *
 * Claiming systems and strings is a way to avoid mission collisions
 * preemptively.
 *
 * Note it does not actually perform the claim if it fails to claim. It also
 *  does not work more than once.
 *
 * @usage if not misn.claim( { system.get("Gamma Polaris") } ) then misn.finish(
 * false ) end
 * @usage if not misn.claim( system.get("Gamma Polaris") ) then misn.finish(
 * false ) end
 * @usage if not misn.claim( 'some_string' ) then misn.finish( false ) end
 * @usage if not misn.claim( { system.get("Gamma Polaris"), 'some_string' } )
