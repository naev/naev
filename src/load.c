...
static player_saves_t *load_player =
   NULL; /**< Points to current element in load_saves. */
...
/* player.c */
extern Spob *
player_load( xmlNodePtr parent ); /**< Loads player related stuff. */
/* event.c */
...
static int  load_enumerateCallback( void *data, const char *origdir,
                                    const char *fname );
static int  load_enumerateCallbackPlayer( void *data, const char *origdir,
                                          const char *fname );
...
int load_refresh( void )
{
   ...
   for ( int i = 0; i < array_size( load_saves ); i++ ) {
      player_saves_t *ps = &load_saves[i];
      qsort( ps->saves, array_size( ps->saves ), sizeof( nsave_t ),
             load_sortCompare );
   }
   ...
   qsort( load_saves, array_size( load_saves ), sizeof( player_saves_t ),
   ...
   for ( int i = array_size( load_saves ) - 1; i > 0; i-- ) {
      ...
      qsort( ps[-1].saves, array_size( ps[-1].saves ), sizeof( nsave_t ),
...
static int load_compatibilityTest( const nsave_t *ns )
{
   ...
   case SAVE_COMPATIBILITY_PLUGINS:
      ...
      for ( int i = 0; i < array_size( plugins ); i++ )
         l += scnprintf( &buf2[l], sizeof( buf2 ) - l, "%s%s",
                         ( l > 0 ) ? p_( "plugins", ", " ) : "",
                         plugin_name( &plugins[i] ) );
      if ( !dialogue_YesNo(
              _( "Save game plugin mismatch" ),
...
void load_loadGameMenu( void )
{
   ...
   /* window */
   wid = window_create( "wdwLoadGameMenu", _( "Load Pilot" ), -1, -1,
   ...
   if ( n > 0 ) {
      ...
      for ( int i = 0; i < n; i++ ) {
         ...
         if ( ns->compatible ) {
            ...
            scnprintf( buf, sizeof( buf ), _( "%s (#r%s#0)" ), ns->player_name,
                       load_compatibilityString( ns ) );
            names[i] = strdup( buf );
         } else
   ...
   /* Player text. */
   window_addText( wid, -20, -40, BUTTON_WIDTH * 2 + 20,
                   LOAD_HEIGHT - 40 - 20 - 2 * ( BUTTON_HEIGHT + 20 ), 0,
                   "txtPilot", &gl_smallFont, NULL, NULL );

   window_addList( wid, 20, -40, LOAD_WIDTH - BUTTON_WIDTH * 2 - 80,
                   LOAD_HEIGHT - 110, "lstNames", names, n, pos,
                   load_menu_update, load_menu_load );
   ...
   if ( old_saves_detected && !player_warned ) {
      ...
      snprintf( buf, sizeof( buf ), "%s%s",
                PHYSFS_getRealDir( "saves-pre-0.10.0" ), "saves-pre-0.10.0" );
      dialogue_alert( _( "Naev has detected saves in pre-0.10.0 format, and "
                         "has automatically migrated them to the new format. "
                         "Old saves have been backed up at '%s'." ),
                      buf );
      player_warned = 1;
   }
}
...
void load_loadSnapshotMenu( const char *name, int disablesave )
{
   ...
   /* window */
   wid = window_create( "wdwLoadSnapshotMenu", _( "Load Snapshot" ), -1, -1,
                        LOAD_WIDTH, LOAD_HEIGHT );
   ...
   *data = disablesave;
   ...
   if ( n > 0 ) {
      ...
      for ( int i = 0; i < n; i++ ) {
         ...
         if ( ns->compatible ) {
            char buf[STRMAX_SHORT];
            scnprintf( buf, sizeof( buf ), _( "%s (#r%s#0)" ), ns->save_name,
                       load_compatibilityString( ns ) );
            names[i] = strdup( buf );
         } else
   ...
   window_addList( wid, 20, -40, LOAD_WIDTH - BUTTON_WIDTH * 2 - 80,
                   LOAD_HEIGHT - 110, "lstSaves", names, n, 0,
                   load_snapshot_menu_update, load_snapshot_menu_load );
...
static void load_snapshot_menu_save( unsigned int wdw, const char *str )
{
   char *save_name = dialogue_input(
      _( "Save game" ), 1, 60, _( "Please give the new snapshot a name:" ) );
   if ( save_name == NULL )
      return;
   char path[PATH_MAX];
...
static void display_save_info( unsigned int wid, const nsave_t *ns )
{
   char buf[STRMAX_SHORT], credits[ECON_CRED_STRLEN], date[64],
      difficulty[STRMAX_SHORT];
   size_t l = 0;

   if ( ns->difficulty == NULL ) {
      const Difficulty *d = difficulty_cur();
      snprintf( difficulty, sizeof( difficulty ), _( "%s (options)" ),
                _( d->name ) );
   } else
      snprintf( difficulty, sizeof( difficulty ), _( "%s (this save)" ),
                _( ns->difficulty ) );
...
static void load_snapshot_menu_load( unsigned int wdw, const char *str )
{
   ...
   if ( load_game( &load_player->saves[pos] ) ) {
      /* Failed so reopen both. */
      /* TODO how to handle failure here? It can happen at many different points
       * now. */
      /*menu_main();
      load_loadGameMenu();*/
   }
...
static void load_menu_delete( unsigned int wdw, const char *str )
{
   ...
   for ( int i = 0; i < n; i++ )
      if ( !PHYSFS_delete( load_saves[pos].saves[i].path ) )
         dialogue_alert( _( "Unable to delete %s" ),
                         load_saves[pos].saves[i].path );
   snprintf( path, sizeof( path ), "saves/%s", load_saves[pos].name );
   if ( !PHYSFS_delete( path ) )
      dialogue_alert( _( "Unable to delete '%s' directory" ),
                      load_saves[pos].name );
...
static int load_gameInternal( const char *file, const char *version )
{
    ...
    */
   if ( ( player.p == NULL ) ||
        player_isFlag( PLAYER_DESTROYED ) ) /* same condition in hook.c */
...
static int load_gameInternalHook( void *data )
{
   ...
   if ( misn_failed || evt_failed ) {
      ...
      l += scnprintf(
         &buf[l], sizeof( buf ) - l,
         _( "Saved game '%s' failed to load some missions/events properly!" ),
         file );
      if ( misn_failed ) {
         l += scnprintf( &buf[l], sizeof( buf ) - l,
                         _( "\nIn particular, the following missions have "
                            "failed to load and been removed:" ) );
      ...
      /*l +=*/scnprintf(
         ...
         _( "\nNote that, in general, you should be able to find the "
            "missions/events again and start them without penalty." ) );
