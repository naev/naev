...
{
   /* Here larger is "better", so we divide by ew_hide and ew_signature instead
    * of multiplying. */
   return pow( p->solid.mass, 1. / 3. ) * 1.25 /
          ( p->stats.ew_hide * p->stats.ew_signature ) *
          p->stats.ew_scanned_time;
}
...
void pilot_ewUpdateDynamic( Pilot *p, double dt )
{
   ...
   if ( vec2_dist2( &p->solid.pos, &t->solid.pos ) <
      ...
      if ( p->scantimer < 0. ) {
         ...
         /* TODO handle case the player finished scanning by setting a flag or
          * something. */
      }
   }
}
 ...
 *    @param[out] dist2 Distance squared of the two pilots. Set to NULL if
 * you're not interested.
 *    @return 1 if they are in range, 0 if they aren't and -1 if they are
 * detected fuzzily.
 */
 ...
 *
 *    @param p Pilot who is trying to check to see if the spob is in sensor
 * range.
 ...
 *
 *    @param p Pilot who is trying to check to see if the asteroid is in sensor
 * range.
 ...
 *
 *    @param p Pilot who is trying to check to see if the jump point is in
 * sensor range.
                            ...
                            double trackmax )
{
   ...
   return CLAMP(
      ...
      ( t->ew_signature * mod - trackmin ) /
         MAX( trackmax - trackmin,
              DOUBLE_TOL ) ); /* Avoid divide by zero if trackmax==trackmin. */
}

/**
 * @brief Checks to see if there are pilots nearby to a stealthed pilot that
 * could break stealth.
 *
 *    @param p Pilot to check.
