 ...
 *    @param keynum Whether the key is a number (not a string) and should be
 * read back as such.
 *    @return 0 on success.
 */
...
static int nxml_saveJump( xmlTextWriterPtr writer, const char *name,
                          size_t name_len, const char *start, const char *dest,
                          int keynum )
{
   xmlw_startElem( writer, "data" );
...
static int nxml_persistDataNode( lua_State *L, xmlTextWriterPtr writer )
{
   ...
   /* key, value */
   /* Handle different types of keys, we must not touch the stack after this
    * operation. */
   switch ( lua_type( L, -2 ) ) {
   case LUA_TSTRING:
   ...
   /* Normal number. */
   case LUA_TNUMBER:
      nxml_saveData( writer, "number", name, name_len, lua_tostring( L, -1 ),
                     keynum );
      /* key, value */
      break;
   ...
   case LUA_TSTRING:
      ...
      if ( nxml_canWriteString( data, len ) )
         nxml_saveData( writer, "string", name, name_len, lua_tostring( L, -1 ),
                        keynum );
      else {
         char *encoded = base64_encode_to_cstr( data, len );
         nxml_saveData( writer, "string_base64", name, name_len, encoded,
                        keynum );
         free( encoded );
      }
      /* key, value */
   ...
   case LUA_TUSERDATA:
      if ( lua_isspob( L, -1 ) ) {
         ...
         if ( pnt != NULL )
            nxml_saveData( writer, SPOB_METATABLE, name, name_len, pnt->name,
                           keynum );
         else
         ...
         if ( ss != NULL )
            nxml_saveData( writer, SYSTEM_METATABLE, name, name_len, ss->name,
                           keynum );
         else
         ...
         if ( !faction_isFaction(
                 lf ) ) /* Dynamic factions may become invalid for saving. */
            break;
         str = faction_name( lua_tofaction( L, -1 ) );
         if ( str == NULL )
            break;
         nxml_saveData( writer, FACTION_METATABLE, name, name_len, str,
                        keynum );
         /* key, value */
         break;
      ...
      if ( xml_isNode( node, "data" ) ) {
         ...
         if ( strcmp( type, "table" ) == 0 ) {
            ...
            if ( pnt != NULL ) {
               ...
               WARN( _( "Failed to load nonexistent spob '%s'" ),
                     xml_get( node ) );
               failed = 1;
            }
            ...
            if ( ss != NULL )
               ...
               WARN( _( "Failed to load nonexistent system '%s'" ),
                     xml_get( node ) );
               failed = 1;
            }
            ...
            if ( ( ss != NULL ) && ( dest != NULL ) ) {
               ...
               WARN( _( "Failed to load nonexistent jump from '%s' to '%s'" ),
                     xml_get( node ), buf );
               failed = 1;
            }
            free( buf );
