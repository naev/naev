...
void gl_beginSolidProgram(mat4 projection, const glColour *c)
{
   ...
   mat4_uniform(shaders.solid.projection, projection);
}
...
void gl_beginSmoothProgram(mat4 projection)
{
   ...
   mat4_uniform(shaders.smooth.projection, projection);
}
      ...
      const glColour *c, double angle )
{
   ...
   mat4_uniform(shaders.texture.projection, projection);
   mat4_uniform(shaders.texture.tex_mat, tex_mat);
      ...
      double tx, double ty, double tw, double th, const glColour *c )
{
   ...
   mat4_uniform(shaders.texture_interpolate.projection, projection);
   mat4_uniform(shaders.texture_interpolate.tex_mat, tex_mat);
