...
impl NLua {
   ...
   pub fn resize(&self, width: i32, height: i32) -> Result<()> {
      for pair in self.envs.pairs::<i32, mlua::Table>() {
         ...
         match resize {
            mlua::Value::Nil => Ok(()),
            mlua::Value::Function(mf) => self.environment_call::<()>(value, &mf, (width, height)),
            _ => {
...
impl LuaEnv {
   ...
   pub fn load_standard(&mut self, lua: &NLua) -> Result<()> {
      ...
      open_lib("collide", collide::open_collide)?;
...
pub extern "C-unwind" fn nlua_pcall(env: *const LuaEnv, nargs: c_int, nresults: c_int) -> c_int {
   ...
   let ret = match wrap(env, nargs, nresults) {
      ...
      Err(mlua::Error::RuntimeError(s)) => {
         unsafe {
            NLUA.lua.exec_raw_lua(|state| {
               if let Err(e) = s.push_into_stack_multi(state) {
                  warn_err!(e);
               }
            });
         }
