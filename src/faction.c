...
typedef struct Faction_ {
   ...
   /* Presence stuff. */
   int generates; /**< Secondary faction generated by this faction. */
   double generates_weight; /**< Percent that bleeds over. */
...
static void faction_parseSocial( xmlNodePtr parent )
{
   ...
   node = parent->xmlChildrenNode;
   do {
      if (xml_isNode(node, "generates")) {
         xmlr_attr_float(node,"weight",base->generates_weight);
         base->generates = faction_get( xml_get(node) );
         continue;
      }
...
int faction_generates( int f, double *weight )
{
   int g;
   if (faction_isFaction(f)) {
      g = faction_stack[f].generates;
      if (g>=0) {
         *weight = faction_stack[f].generates_weight;
         return g;
      }
   }
   return -1;
