...
static char *gettext_systemLanguage =
   NULL; /**< Language, or :-delimited list of them, from the system at startup.
          */
static translation_t *gettext_translations =
   NULL; /**< Linked list of loaded translation chains. */
static translation_t *gettext_activeTranslation =
   NULL; /**< Active language's code. */
static uint32_t gettext_nstrings =
   0; /**< Number of translatable strings in the game. */
 ...
 * There's no presumption that PhysicsFS is available, so this doesn't actually
 * load translations. We loosely follow:
...
void gettext_init( void )
{
   ...
   setlocale( LC_ALL, "" );
   /* If we don't disable LC_NUMERIC, lots of stuff blows up because 1,000 can
    * be interpreted as 1.0 in certain languages. */
 ...
 * This invalidates previously returned pointers to translated strings; be sure
 * to actually exit after calling!
 */
...
void gettext_setLanguage( const char *lang )
{
   ...
   if ( gettext_activeTranslation != NULL &&
        !strcmp( lang, gettext_activeTranslation->language ) )
      return;

   /* Search for the selected language in the loaded translations. */
   for ( translation_t *ptrans = gettext_translations; ptrans != NULL;
         ptrans                = ptrans->next )
   ...
   while ( lang != NULL && *lang != 0 ) {
      ...
      lang_match =
         gettext_matchLanguage( lang_part, lang_part_len, available_langs );
      if ( lang_match == NULL )
         continue;
                                          ...
                                          char *const *available )
{
   ...
   /* Good enough for now: Return the greatest (thus longest) string matching up
    * to their common length. */
   for ( size_t i = 0; available[i] != NULL; i++ ) {
      int c =
         strncmp( lang, available[i], MIN( lang_len, strlen( available[i] ) ) );
      if ( c < 0 )
         break;
      else if ( c == 0 )
         best = available[i];
 ...
 * (Common case: just a "naev.txt" file with one number. But mods pulled in via
 * PhysicsFS can have their own string counts.)
 */
static void gettext_readStats( void )
{
...
/**
 * @brief Return the fraction of strings which have a translation into the given
 * language.
 *
...
/* The function is almost the same as p_() but msgctxt and msgid can be string
 * variables.
 */
