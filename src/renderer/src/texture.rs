                     ...
                     .and_then(|s| s.to_str())
                  {
                     ...
                     None => {
                        ...
                        if image.is_none() {
                           ...
                           if ndata::exists(path) {
                              image = Some(svg_to_img(&mut ndata::iostream(path)?, w, h)?);
                           }
                        }
                        image
                           .with_context(|| format!("No image file matching '{}' found", cpath))?
                     }
                  }
               };
               let ctx = &sctx.lock();
...
pub extern "C" fn gl_newSpriteRWops(
   ...
   builder = if cpath.is_null() {
      let rw =
         unsafe { sdl::iostream::IOStream::from_ll(rw as *mut sdl::sys::iostream::SDL_IOStream) };
      builder.iostream(rw)
   } else {
      let path = unsafe { CStr::from_ptr(cpath) };
      let pathname = path.to_string_lossy();
...
impl<'de> DeserializeSeeded<'de, TextureDeserializer<'_>> for Texture {
   fn deserialize_seeded<D>(loader: &TextureDeserializer, deserializer: D) -> Result<Self, D::Error>
   where
      D: serde::Deserializer<'de>,
   {
      let name = String::deserialize(deserializer)?;
