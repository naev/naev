...
impl Audio {
   ...
   fn try_clone(&self, source: al::Source) -> Result<Self> {
      match self {
         Self::Static(this) | Self::LuaStatic(this) => Ok(Audio::Static(this.try_clone(source)?)),
...
impl System {
   pub fn new() -> Result<Self> {
      ...
      if has_output_limiter && device.get_parameter_i32(ALC_OUTPUT_LIMITER_SOFT) != ALC_TRUE as i32
      {
   ...
   fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
            ...
            this.call_or(
               |this| {
                  ...
                  let slot = if enable {
                     ...
                     let efxid =
                        match binary_search_by_key_ref(&lock, name.deref(), |e: &LuaAudioEfx| {
                           &e.name
                        }) {
                           Ok(efx) => efx,
                           Err(_) => {
      ...
      methods.add_function(
         ...
         |_, (name, param): (BorrowedStr, mlua::Table)| -> mlua::Result<()> {
            ...
            let efxid = match binary_search_by_key_ref(&lock, name.deref(), |e: &LuaAudioEfx| {
               &e.name
            }) {
               Ok(efx) => efx,
               Err(_) => {
                  ...
                  binary_search_by_key_ref(&lock, name.deref(), |e: &LuaAudioEfx| &e.name).unwrap()
               }
            };
            let efx = &lock[efxid];
            ...
            macro_rules! efx_set_f32 {
               ($name: literal, $field: ident) => {{
                  match param.get::<mlua::Value>($name)? {
                     mlua::Value::Nil => (),
                     mlua::Value::Number(val) => efx.effect.parameter_f32($field, val as f32),
                     mlua::Value::Integer(val) => efx.effect.parameter_f32($field, val as f32),
                     val => {
                        return Err(mlua::Error::RuntimeError(format!(
            ...
            macro_rules! efx_set_i32 {
               ($name: literal, $field: ident) => {{
                  match param.get::<mlua::Value>($name)? {
                     mlua::Value::Nil => (),
                     mlua::Value::Number(val) => efx.effect.parameter_i32($field, val as i32),
                     mlua::Value::Integer(val) => efx.effect.parameter_i32($field, val as i32),
                     val => {
                        return Err(mlua::Error::RuntimeError(format!(
         ...
         |_, name: Option<BorrowedStr>| -> mlua::Result<()> {
            if let Some(efx) = EFX.get() {
               ...
               if let Some(name) = name {
                  let lock = EFX_LIST.lock().unwrap();
                  let efxid =
                     match binary_search_by_key_ref(&lock, name.deref(), |e: &LuaAudioEfx| &e.name)
                     {
                        Ok(efx) => efx,
                        Err(_) => {
                           return Err(mlua::Error::RuntimeError(format!(
