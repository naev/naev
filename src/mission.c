...
static char       **player_missions_failed =
   NULL; /**< Name of missions that failed to load. */
...
static int  mission_init( Mission *mission, const MissionData *misn, int genid,
...
{
   if ( ( id < 0 ) || ( id >= array_size( mission_stack ) ) )
      return NULL;
                         ...
                         int create, unsigned int *id )
{
   if ( misn->chunk == LUA_NOREF ) {
      WARN(
         _( "Trying to initialize mission '%s' that has no loaded Lua chunk!" ),
         misn->name );
      return -1;
   }
   ...
   /* init Lua */
   mission->env = nlua_newEnv( misn->name );
...
static int mission_meetConditionals( const MissionData *misn )
{
   ...
   if ( ( misn->avail.done != NULL ) &&
        ( player_missionAlreadyDone( mission_getID( misn->avail.done ) ) ==
          0 ) )
      return 1;
                            ...
                            const Spob *pnt, const StarSystem *sys )
{
   ...
   if ( ( misn->avail.system != NULL ) &&
        ( sys == NULL || ( strcmp( misn->avail.system, sys->name ) != 0 ) ) )
      return 0;
                   ...
                   const StarSystem *sys )
{
   for ( int i = 0; i < array_size( mission_stack ); i++ ) {
      ...
      if ( RNGF() < chance ) {
         ...
         mission_cleanup(
            &mission ); /* it better clean up for itself or we do it */
      }
   }
}
...
static int mission_markerLoad( Mission *misn, xmlNodePtr node )
{
   ...
   case SYSMARKER_PLOT:
      ...
      if ( ssys == NULL ) {
         WARN( _( "Mission Marker to system '%s' does not exist" ),
               xml_get( node ) );
         return -1;
      }
   ...
   case SPOBMARKER_PLOT:
      ...
      if ( pnt == NULL ) {
         WARN( _( "Mission Marker to spob '%s' does not exist" ),
               xml_get( node ) );
         return -1;
      }
 ...
 */
int mission_addMarker( Mission *misn, int id, int objid,
                       MissionMarkerType type )
{
...
/**
 * @brief Marks the system of the computer mission to reflect where it will head
 * to.
 *
...
void mission_cleanup( Mission *misn )
{
   ...
   if ( ( player.p != NULL ) &&
        !pilot_isFlag( player.p,
                       PILOT_DEAD ) ) { /* Only remove if player exists. */
      for ( int i = 0; i < array_size( misn->cargo );
            i++ ) { /* must unlink all the cargo */
         int ret = pilot_rmMissionCargo( player.p, misn->cargo[i], 0 );
         if ( ret )
            WARN( _( "Failed to remove mission cargo '%d' for mission '%s'." ),
                  misn->cargo[i], misn->title );
      }
   }
...
static int mission_parseXML( MissionData *temp, const xmlNodePtr parent )
{
   ...
   do { /* load all the data */
      ...
      if ( xml_isNode( node, "location" ) ) {
         ...
         if ( temp->avail.loc < 0 )
            WARN( _( "Mission '%s' has unknown location '%s'!" ), temp->name,
                  xml_get( node ) );
         continue;
      }
      ...
      if ( xml_isNode( node, "tags" ) ) {
         ...
         do {
            ...
            WARN( _( "Mission '%s' has unknown node in tags '%s'." ),
                  temp->name, cur->name );
   ...
   if ( temp->avail.cond != NULL ) {
      ...
      if ( temp->avail.cond_chunk == LUA_NOREF ||
           temp->avail.cond_chunk == LUA_REFNIL )
         WARN( _( "Mission '%s' failed to compile Lua conditional!" ),
               temp->name );
   }
...
#define MELEMENT( o, s )                                                       \
   ...
   MELEMENT( ( temp->avail.loc != MIS_AVAIL_NONE ) &&
                ( temp->avail.chance < 0. ),
             "chance" );
   MELEMENT(
      ( ( temp->avail.spob != NULL ) && spob_get( temp->avail.spob ) == NULL ),
      "spob" );
   MELEMENT( ( ( temp->avail.system != NULL ) &&
               system_get( temp->avail.system ) == NULL ),
             "system" );
#undef MELEMENT
...
int missions_load( void )
{
    ...
    * first. */
   qsort( mission_stack, array_size( mission_stack ), sizeof( MissionData ),
          missions_cmp );

#if DEBUGGING
   if ( conf.devmode ) {
      time = SDL_GetTicks() - time;
      DEBUG( n_( "Loaded %d Mission in %.3f s", "Loaded %d Missions in %.3f s",
                 array_size( mission_stack ) ),
             array_size( mission_stack ), time / 1000. );
   } else
      DEBUG( n_( "Loaded %d Mission", "Loaded %d Missions",
                 array_size( mission_stack ) ),
             array_size( mission_stack ) );
#endif /* DEBUGGING */
...
static int missions_parseActive( xmlNodePtr parent )
{
   ...
   do {
      if ( xml_isNode( node, "mission" ) ) {
         ...
         if ( data == NULL ) {
            WARN( _( "Mission '%s' from saved game not found in game - "
                     "ignoring." ),
                  buf );
            ...
            if ( mission_init( misn, data, 0, 0, NULL ) ) {
               WARN( _( "Mission '%s' from saved game failed to load properly "
                        "- ignoring." ),
                     buf );
               array_push_back( &player_missions_failed, strdup( buf ) );
               free( buf );
         ...
         do {
            ...
            if ( xml_isNode( cur, "osd" ) ) {
               ...
               do {
                  if ( xml_isNode( nest, "msg" ) ) {
                     if ( i > nitems ) {
                        WARN(
                           _( "Inconsistency with 'nitems' in save file." ) );
                        break;
                     }
            ...
            if ( xml_isNode( cur, "lua" ) ) {
               ...
               if ( ret ) {
                  WARN( _( "Mission '%s' from saved game failed to unpersist "
                           "Lua properly - ignoring." ),
                        data->name );
                  misn_failed = -1;
               }
            }
