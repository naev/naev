...
typedef struct LandOutfitData_ {
   const Outfit *
      *outfits; /**< Override list of outfits, otherwise uses landed_spob. */
...
static Outfit *
   *iar_outfits[OUTFITS_NTABS]; /**< C-array of Arrays: Outfits associated with
                                   the image array cells. */
static int             outfit_Mode = 0; /**< Outfit mode for filtering. */
static PlayerOutfit_t *outfits_sold =
   NULL; /**< List of the outfits the player sold so they can buy them back. */
 ...
 * Helper functions.
 */
static void outfits_getSize( unsigned int wid, int *w, int *h, int *iw, int *ih,
                             int *bw, int *bh );
...
void outfits_open( unsigned int wid, const Outfit **outfits, int blackmarket )
{
   ...
   if ( data->outfits == NULL ) {
      window_addButtonKey( wid, off, 20, bw, bh, "btnCloseOutfits",
   ...
   off -= 20 + bw;
   window_addButtonKey( wid, off, 20, bw, bh, "btnSellOutfit", _( "Sell" ),
                        outfits_sell, SDLK_S );
   off -= 20 + bw;
   ...
   window_addText( wid, 20 + iw + 20, 0, 90, 160, 0, "txtSDesc", &gl_defFont,
                   &cFontGrey, NULL );
   window_addText( wid, 20 + iw + 20 + 90, 0, w - ( 20 + iw + 20 + 90 ), 160, 0,
                   "txtDDesc", &gl_defFont, NULL, NULL );
   window_addText( wid, 20 + iw + 20, 0, w - ( iw + 80 ),
                   h, /* TODO: Size exactly and resize instead of moving? */
...
void outfits_regenList( unsigned int wid, const char *str )
{
   ...
   /* Must exist. */
   if ( landed && ( !land_tabGenerated( LAND_WINDOW_OUTFITS ) ||
                    ( ( data->outfits == NULL ) &&
...
static void outfits_genList( unsigned int wid )
{
   int ( *tabfilters[] )( const Outfit *o ) = {
      outfitLand_filter,        outfitLand_filterWeapon,
      outfitLand_filterUtility, outfitLand_filterStructure,
      outfitLand_filterCore,    outfit_filterOther,
      outfitLand_filter,
   };
   ...
   if ( !widget_exists( wid, OUTFITS_TAB ) ) {
      ...
      const char *tabnames[] = {
         _( "All" ),
         _( OUTFIT_LABEL_WEAPON ),
         _( OUTFIT_LABEL_UTILITY ),
         _( OUTFIT_LABEL_STRUCTURE ),
         _( OUTFIT_LABEL_CORE ),
         _( "#rOther" ),
         _( "Owned" ),
      };
      ...
      if ( iw >= 65 ) {
         ...
         window_addButton( wid, fx + fw - 30, fy, 30, 30, "btnOutfitFilter",
                           NULL, outfit_Popdown );
         ...
         /* Set text filter. */
         window_addInput( wid, fx, fy, fw, fh, OUTFITS_FILTER, 32, 1,
                          &gl_defFont );
   ...
   if ( active == 6 ) {
      ...
      qsort( iar_outfits[active], array_size( iar_outfits[active] ),
             sizeof( Outfit * ), outfit_compareTech );
   } else {
   ...
   coutfits = outfits_imageArrayCells( (const Outfit **)iar_outfits[active],
                                       &noutfits, player.p, 1 );

   iconsize = 128;
   if ( !conf.big_icons ) {
      if ( toolkit_simImageArrayVisibleElements( iw, ih - 34, iconsize,
                                                 iconsize ) < noutfits )
         iconsize = 96;
      if ( toolkit_simImageArrayVisibleElements( iw, ih - 34, iconsize,
                                                 iconsize ) < noutfits )
   ...
   window_addImageArray( wid, 20, 20, iw, ih - 34, OUTFITS_IAR, iconsize,
                         iconsize, coutfits, noutfits, outfits_update,
                         outfits_rmouse, NULL );
...
void outfits_update( unsigned int wid, const char *str )
{
   ...
   if ( i < 0 || array_size( iar_outfits[active] ) == 0 ) { /* No outfits */
      ...
      window_moveWidget( wid, "txtDDesc", 20 + iw + 20 + 90,
                         -40 - th - 30 - 32 );
...
ImageArrayCell *outfits_imageArrayCells( const Outfit **outfits, int *noutfits,
                                         const Pilot *p, int store )
{
   ImageArrayCell *coutfits =
      calloc( MAX( 1, *noutfits ), sizeof( ImageArrayCell ) );
...
static void outfit_Popdown( unsigned int wid, const char *str )
{
   ...
   window_addList( wid, x + w, y - 120 + h, 350, 120, name, modelist, n,
                   outfit_Mode, outfit_PopdownSelect, outfit_PopdownActivate );
...
int outfit_canBuy( const Outfit *outfit, int wid )
{
   ...
   if ( outfit_isProp( outfit, OUTFIT_PROP_UNIQUE ) &&
        ( player_outfitOwnedTotal( outfit ) > 0 ) ) {
   ...
   if ( ( outfit_isMap( outfit ) && map_isUseless( outfit ) ) ||
      ...
      land_errDialogueBuild(
         _( "You already know of everything this map contains." ) );
      return 0;
   }
   /* GUI already owned */
...
int outfit_canSell( const Outfit *outfit )
{
   ...
   if ( !pilot_hasIntrinsic( player.p, outfit ) &&
        ( player_outfitOwned( outfit ) <= 0 ) ) {
