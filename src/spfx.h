...
#define SPFX_DAMAGE_DECAY                                                      \
   0.5 /**< Rate at which the damage strength goes down. */
#define SPFX_DAMAGE_MOD                                                        \
   1.5 /**< How damage spfx gets modified (base is armour_dmg / total_armour   \
        */
...
/**
 * @brief IDs for the type of emission. (It's modal: one trail can have segments
 * of different types.)
 */
...
#define MODE_TAGS                                                              \
   {                                                                           \
      "idle", "glow", "afterburn", "jumping", "none",                          \
   }
...
typedef struct TrailSpec_ {
   ...
   double accel_base; /**< Fixed value that is added when accelerating (defaults
                         to 0.). */
...
typedef struct Trail_spfx_ {
   ...
   TrailPoint
      *point_ringbuf; /**< Circular buffer (malloced/freed) of trail points. */
   ...
   unsigned int ontop; /**< Boolean to decide if the trail is drawn before or
                          after the ship. */
} Trail_spfx;

/** @brief Indexes into a trail's circular buffer.  */
#define trail_at( trail, i )                                                   \
   ( ( trail )->point_ringbuf[( i ) & ( ( trail )->capacity - 1 )] )
