...
static int pilot_cargoAddInternal( Pilot *pilot, CommodityRef cargo,
                                   int quantity, unsigned int id );

/* ID Generators. */
static unsigned int mission_cargo_id =
   0; /**< ID generator for special mission cargo.
 ...
 */
int pilot_cargoOwned( const Pilot *pilot, CommodityRef cargo )
{
 ...
 *
 * At the end has `dest` have exactly the same cargo as `src` and leaves `src`
 * with none.
 *
...
int pilot_cargoMove( Pilot *dest, Pilot *src )
{
   ...
   if ( pilot_cargoUsed( src ) > pilot_cargoFree( dest ) ) {
      WARN( _( "Unable to copy cargo over from pilot '%s' to '%s'. Leaving "
               "cargo as is." ),
            src->name, dest->name );
      return -1;
   }
 ...
 *    @param id Mission ID to add (0 is none).
 */
int pilot_cargoAddRaw( Pilot *pilot, CommodityRef cargo, int quantity,
                       unsigned int id )
{
 ...
 *
 * Note that the commodity will not be added as 0 quantity unless 0 is
 * explicitly specified, or it is mission cargo.
 *
 *    @param pilot Pilot to add cargo to.
 *    @param cargo Cargo to add.
 *    @param quantity Quantity to add.
 ...
 */
int pilot_cargoAdd( Pilot *pilot, CommodityRef cargo, int quantity,
                    unsigned int id )
{
 ...
 */
unsigned int pilot_addMissionCargo( Pilot *pilot, CommodityRef cargo,
                                    int quantity )
{
...
int pilot_rmMissionCargo( Pilot *pilot, unsigned int cargo_id, int jettison )
{
   ...
   pilot->solid.mass -=
      pilot->stats.cargo_inertia * pilot->commodities[i].quantity;
   array_erase( &pilot->commodities, &pilot->commodities[i],
                &pilot->commodities[i + 1] );
...
/**
 * @brief Tries to get rid of quantity cargo from pilot. Can remove mission
 * cargo.
 *
 ...
 */
int pilot_cargoRmRaw( Pilot *pilot, CommodityRef cargo, int quantity,
                      int cleanup )
{
   /* Check if pilot has it */
   int q = quantity;
   for ( int i = 0; i < array_size( pilot->commodities ); i++ ) {
      ...
      pilot->solid.mass -= pilot->stats.cargo_inertia * q;
      pilot_updateMass( pilot );
      /* This can call Lua code and be called during takeoff (pilot cleanup),
       * causing the Lua code to be run with a half-assed pilot state crashing
       * the game. */
      if ( !cleanup )
         gui_setGeneric( pilot );
      return q;
...
int pilot_cargoRmAll( Pilot *pilot, int cleanup )
{
    ...
    * Caution: it could make sense to communicate a deletion, but particularly
    * in the middle of pilots_clean() that's unsafe. */
   if ( !cleanup )
      gui_setGeneric( pilot );
   return q;
}
 ...
 *    @return Amount of cargo gotten rid of.
 */
int pilot_cargoRm( Pilot *pilot, CommodityRef cargo, int quantity )
{
 ...
 *    @return Amount of cargo gotten rid of.
 */
int pilot_cargoJet( Pilot *p, CommodityRef cargo, int quantity, int simulate )
{
   int    n;
