 ...
 *
 * @note Not to be confused with getting the player's ship that can be done with
 * player.pilot():ship().
 *
 ...
 *    @luatparam[opt] boolean|string nohooks Set to true to not trigger pay
 * hooks, or a string value to pass that to the pay hook instead.
 * @luafunc pay
 */
static int playerL_pay( lua_State *L )
{
   ...
   money = CLAMP( CREDITS_MIN, CREDITS_MAX,
                  (credits_t)round( luaL_checknumber( L, 1 ) ) );
 ...
 *
 *    @luatparam[opt] number decimal Optional argument that makes it return
 * human readable form with so many decimals.
...
/**
 * @brief Gets how many credits the player owns both directly, and in the form
 * of assets (ships, outfits, ...).
 *
 ...
 *    @luatparam[opt=false] boolean display Display the message over the
 * player's ship (like a broadcast message).
 * @luafunc msg
 */
static int playerL_msg( lua_State *L )
{
 ...
 *    @luatparam[opt=10] number duration Duration to add message in seconds (if
 * 0. the duration is infinite).
 *    @luatparam[opt=16] number fontsize Size of the font to use.
 *    @luatparam[opt=white] Colour col Colour to use for the text or white if
 * not specified.
 ...
 *    @luatparam number duration New duration to set in seconds (0. for
 * infinity).
 ...
 *    @luatparam[opt=true] boolean b true if the player is allowed to save,
 * false otherwise.
 * @luafunc allowSave
 */
static int playerL_allowSave( lua_State *L )
...
static int playerL_getPilot( lua_State *L )
{
   /* No need to run check here or stuff that depends on player.pilot() working
    * will fail. */
 ...
 *
 *    @luatparam Vec2|nil pos Position to mark as autonav destination or nil to
 * clear.
 ...
 * @note Does not do anything if the player is not in autonav.
 *
 *    @luatparam[opt=0.] number timer How many seconds to wait before starting
 * autonav up again.
 ...
 *
 *    @luatparam number speed Speed to set the game to. If omitted it will reset
 * the game speed.
 *    @luatparam[opt=speed] number sound Sound speed to set to.
 *    @luatparam[opt=false] boolean noset Avoid setting player.speed. Useful for
 * autonav.
 * @luafunc setSpeed
 */
static int playerL_setSpeed( lua_State *L )
...
static int playerL_cinematics( lua_State *L )
{
   ...
   if ( !lua_isnoneornil( L, 2 ) ) {
      if ( !lua_istable( L, 2 ) )
         return NLUA_ERROR( L, _( "Second parameter to cinematics should be a "
                                  "table of options or omitted!" ) );
 ...
 *
 *    @luatparam number mod How strong the effect should be. It should be a
 * value between 0 and 1, usually corresponding to how much armour damage the
 * player has taken with respect to their total armour.
 * @luafunc damageSPFX
 */
static int playerL_damageSPFX( lua_State *L )
{
...
static int playerL_takeoff( lua_State *L )
{
   ...
   if ( !pilot_isSpaceworthy( player.p ) )
      return NLUA_ERROR( L,
                         _( "Player must be spaceworthy to force takeoff!" ) );
 ...
 *
 * Note that this will teleport the player to the system in question as
 * necessary.
 *
 ...
 * @usage player.landAllow( false, "No landing." ) -- Doesn't allow the player
 * to land with the message "No landing."
 *
 *    @luatparam[opt=true] boolean b Whether or not to allow the player to land.
 *    @luatparam[opt] string msg Message displayed when player tries to land
 * (only if disallowed to land). Can be omitted to use default.
 * @luafunc landAllow
 */
static int playerL_landAllow( lua_State *L )
{
 ...
 * Defaults to player's current ship.
 * @luafunc shipvarPeek
 */
static int playerL_shipvarPeek( lua_State *L )
{
 ...
 *    @luatparam[opt] string shipname Name of the ship to push variable to.
 * Defaults to player's current ship.
 * @luafunc shipvarPush
 */
static int playerL_shipvarPush( lua_State *L )
 ...
 *    @luatparam[opt] string shipname Name of the ship to pop variable from.
 * Defaults to player's current ship.
 * @luafunc shipvarPop
 */
static int playerL_shipvarPop( lua_State *L )
 ...
 *
 *   @luatparam[nil=Current ship] string name Name of the ship to get the
 * outfits of.
 ...
 * @usage player.outfitAdd( "Plasma Blaster", 2 ) -- Gives the player two plasma
 * blasters
 *
   ...
   if ( lua_isstring( L, 1 ) ) {
      ...
      if ( strcmp( str, "all" ) == 0 ) {
         ...
         const Outfit        **outfits =
            array_create_size( const Outfit *, array_size( poutfits ) );
 ...
 *
 * @note Should be given when landed, ideally on a spob with a shipyard.
 * Furthermore, this invalidates all player.pilot() references.
 *
 * @usage player.shipAdd( "Pirate Kestrel", _("Seiryuu") ) -- Gives the
...
static int playerL_shipAdd( lua_State *L )
{
   ...
   const char *name     = luaL_optstring( L, 2, ship_name( s ) );
 ...
 *
 * @note You shouldn't use this directly unless you know what you are doing. If
 * the player's cargo doesn't fit in the new ship, it won't be moved over and
 * can lead to a whole slew of issues.
 * @note This invalidates all player.pilot() references!
 *
 *    @luatparam string ship Name of the ship to swap to. (this is name given by
 * the player, not ship name)
 *    @luatparam[opt=false] boolean ignore_cargo Whether or not to ignore cargo
 * and not move it, or try to move it over.
 *    @luatparam[opt=false] boolean remove If true removes the player's current
 * ship (so it replaces and doesn't swap).
 * @luafunc shipSwap
 */
static int playerL_shipSwap( lua_State *L )
{
 ...
 *
 * @usage if player.misnActive( "The Space Family" ) then -- Player is doing
 * space family mission
 *
 *    @luatparam string name Name of the mission to check.
 *    @luatreturn boolean|number Number of instances if the mission is active,
 * false if it isn't.
 * @luafunc misnActive
 */
static int playerL_misnActive( lua_State *L )
 ...
 *
 * @usage if player.misnDone( "The Space Family" ) then -- Player finished
 * mission
 *    @luatparam string name Name of the mission to check.
 ...
 *
 * @usage if player.evtActive( "Shipwreck" ) then -- The shipwreck event is
 * active
 *
 *    @luatparam string name Name of the mission to check.
 ...
 *
 *    @luatreturn table Table containing the names of all the GUIs the player
 * can use.
 * @luafunc guiList
 */
static int playerL_guiList( lua_State *L )
 ...
 *
 *    @luatreturn table Table containing the pilots in the player fleet or false
 * if they are not spawned yet.
 * @luafunc fleetList
 */
static int playerL_fleetList( lua_State *L )
{
   ...
   for ( int i = 0; i < array_size( pstack ); i++ ) {
      ...
      if ( !ps->deployed )
         continue;
      /* We can avoid the spaceWorthy check as they will be set to false due to
       * not having a pilot. */
...
/**
 * @brief Gets the amount of cargo space used by a specific commodity in the
 * player's fleet.
 *
 *    @luatparam Commodity c Commodity to check how much the player has in their
 * fleet.
...
/**
 * @brief Tries to remove an amount of commodity to the player's fleet and
 * jettisons it into space.
 *
 *    @luatparam Commodity c Commodity to remove from the player fleet.
 *    @luatparam number q Amount to remove.
 *    @luatreturn number Amount of commodity removed from the player fleet.
 ...
 *
 *    @luatreturn table A table containing table entries of the form {c =
 * commodity, q = quantity }.
 ...
 *
 * If the destination is a system, the coordinates of the player will not
 * change. If the destination is a spob, the player will be placed over that
 * spob.
 *
 * @usage player.teleport( system.get("Arcanis") ) -- Teleports the player to
 * Arcanis.
 * @usage player.teleport( "Arcanis" ) -- Teleports the player to Arcanis.
 * @usage player.teleport( "Dvaer Prime" ) -- Teleports the player to Dvaer, and
 * relocates him to Dvaer Prime.
 * @usage player.teleport( "Sol", true ) -- Teleports the player to SOL, but
 * doesn't run any initial simulation
 *
 *    @luatparam System|Spob|string dest System or name of a system or spob or
 * name of a spob to teleport the player to.
 *    @luatparam[opt=false] boolean no_simulate Don't simulate the when
 * teleporting if true.
 *    @luatparam[opt=false] boolean silent Doesn't display any entering system
 * messages.
 * @luafunc teleport
 */
static int playerL_teleport( lua_State *L )
{
   ...
   if ( comm_isOpen() )
      return NLUA_ERROR(
         L, _( "Can not teleport the player while the comm is open!" ) );
   if ( player_isBoarded() )
      return NLUA_ERROR(
   ...
   /* Get destination from string. */
      ...
      if ( sysname == NULL ) {
         ...
         if ( pnt == NULL )
            return NLUA_ERROR(
               L, _( "'%s' is not a valid teleportation target." ), name );
...
static int playerL_fleetCapacity( lua_State *L )
{
   ...
   lua_pushboolean( L, ( nships == 0 ) ||
                          ( player.fleet_used <= player.fleet_capacity ) );
   return 3;
}
...
/**
 * @brief Global toggle to whether or not the player can discover space objects
 * and jumps. Meant to be used with cutscenes that use player.teleport().
 *
 ...
 *    @luatparam[opt=nil] Spob|string Spob or name of spob to save the player
 * at.
 * @luafunc save
 */
static int playerL_save( lua_State *L )
{
   ...
   if ( !landed && ( savespob == NULL ) )
      return NLUA_ERROR( L, _( "Unable to save when not landed and land spob "
                               "is not specified!" ) );
   else if ( landed && ( savespob != NULL ) )
      return NLUA_ERROR( L, _( "Unable to save when landed and land_spob does "
                               "not match landed spob!" ) );
...
static int playerL_saveBackup( lua_State *L )
{
   ...
   const char *filename =
      luaL_checkstring( L, 1 ); /* TODO sanitize path and such. */
   ...
   snprintf( backup, sizeof( backup ), "saves/%s/%s.ns", player.name,
             filename );
