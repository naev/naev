 ...
 */
/* Creation. */
static void equipment_getDim( unsigned int wid, int *w, int *h, int *sw,
                              int *sh, int *ow, int *oh, int *ew, int *eh,
...
static void equipment_renderSlots( double bx, double by, double bw, double bh,
                                   void *data );
static void equipment_renderMisc( double bx, double by, double bw, double bh,
                                  void *data );
static void equipment_renderOverlayColumn( double x, double y, double h,
                                           PilotOutfitSlot *lst, int mover,
                                           CstSlotWidget *wgt );
static void equipment_renderOverlaySlots( double bx, double by, double bw,
                                          double bh, void *data );
...
static int  equipment_mouseInColumn( const PilotOutfitSlot *lst, double y,
                                     double h, double my );
...
static int  equipment_swapSlot( unsigned int wid, Pilot *p,
...
static void equipment_changeTab( unsigned int wid, const char *wgt, int old,
                                 int tab );
...
void equipment_rightClickOutfits( unsigned int wid, const char *str )
{
   ...
   for ( int i = 0; i < n; i++ ) {
      ...
      if ( ( size == OUTFIT_SLOT_SIZE_NA ) ||
           ( slots[i].sslot->slot.size < size ) ) {
...
void equipment_open( unsigned int wid )
{
   ...
   /* Get dimensions. */
   equipment_getDim( wid, &w, &h, &sw, &sh, &ow, &oh, &ew, &eh, &cw, &ch, &bw,
                     &bh );
   ...
   x = 20 + sw + 20 + 180 + 10;
   y = -40;
   window_addText( wid, x, y, 130, y - 20 + h - bh, 0, "txtSDesc", &gl_defFont,
                   &cFontGrey, NULL );
   window_addText( wid, x, y, w - x - 128 - 30, y - 20 + h - bh, 0,
                   "txtAcquired", &gl_defFont, NULL, NULL );
   x += 130;
   window_addText( wid, x, y, w - x - 20 - 128 - 20, y - 20 + h - bh, 0,
                   "txtDDesc", &gl_defFont, NULL, NULL );

   /* Generate lists. */
   window_addText( wid, 30, -20, ow, gl_defFont.h, 0, "txtShipTitle",
                   &gl_defFont, NULL, _( "Available Ships" ) );
   window_addText( wid, 30, -40 - sh - 20, ow, gl_defFont.h, 0,
                   "txtOutfitTitle", &gl_defFont, NULL,
                   _( "Available Outfits" ) );

   /* Favourite checkbox, run before genLists. */
   x = -20 - ( 128 - cw ) / 2;
   y = -20 - 150 - ch;
   window_addCheckbox( wid, x, y, cw, 30, "chkFav", _( "Favourite" ),
                       equipment_toggleFav, 0 );
   if ( player.fleet_capacity > 0 ) {
      y -= 23;
      window_addCheckbox( wid, x, y, cw, 30, "chkDeploy", _( "Deployed" ),
                          equipment_toggleDeploy, 0 );
   }
   x = -16;
   y -= 23 + 10;
   ...
   window_addRect( wid, -20 + 4, -40 + 4, 128 + 8, 128 + 8, "rctShip", &cBlack,
                   1 );
   window_addCust( wid, -20, -40, 128, 128, "cstShip", 0, equipment_renderShip,
                   NULL, NULL, NULL, swd );
                           ...
                           double h, CstSlotWidget *data )
{
   ...
   /* Create the widget. */
   window_addCust( wid, x, y, w, h, "cstEquipment", 0, equipment_renderSlots,
                   equipment_mouseSlots, NULL, NULL, data );
                                    ...
                                    const CstSlotWidget *wgt )
{
   ...
   if ( ( o != NULL ) && ( lst[0].sslot->slot.type == outfit_slotType( o ) ) )
      ...
      c = &cFontWhite;
   gl_printMidRaw( &gl_smallFont, 60., x - 15., y + h + 10., c, -1., txt );
   ...
   for ( int i = 0; i < array_size( lst ); i++ ) {
      ...
      if ( lst[i].sslot->locked && !lst[i].sslot->visible &&
           ( lst[i].outfit == NULL ) )
         continue;
      ...
      /* Draw background. */
      if ( ( wgt->weapons >= 0 ) &&
           ( !cantoggle || pilot_weapSetCheck( p, wgt->weapons, &lst[i] ) ) )
         bc = cFontGrey;
      else
         bc = *dc;
      bc.a = 0.4;
                                      ...
                                      double *w, double *h, int *n, int *m )
{
   ...
   /* Calculate size. */
   tm = MAX(
      MAX( array_size( p->outfit_weapon ), array_size( p->outfit_utility ) ),
                                   ...
                                   void *data )
{
   ...
   equipment_renderColumn( x, y, w, h, p->outfit_structure, _( "Structure" ),
   ...
   x -= tw;
   y = by + bh - ( h + 20 ) + ( h + 20 - h ) / 2;
   equipment_renderColumn( x, y, w, h, p->outfit_utility, _( "Utility" ),
                           selected, wgt->outfit, p, wgt );
   ...
   x -= tw;
   y = by + bh - ( h + 20 ) + ( h + 20 - h ) / 2;
   equipment_renderColumn( x, y, w, h, p->outfit_weapon, _( "Weapon" ),
                           selected, wgt->outfit, p, wgt );
}
/**
                                  ...
                                  void *data )
{
   ...
   /* Fleet capacity. */
   if ( player.fleet_capacity > 0 ) {
      gl_printMidRaw( &gl_smallFont, w, x, y + h + 7, &cFontWhite, -1.,
                      _( "Fleet Capacity" ) );
   ...
   /* Render mass limit. */
   gl_printMidRaw( &gl_smallFont, w, x, y - 3, &cFontWhite, -1.,
                   _( "Mass Limit Left" ) );
   y -= gl_smallFont.h + h;

   percent = ( p->stats.engine_limit > 0 )
                ? CLAMP( 0., 1.,
                                          ...
                                          double bh, void *data )
{
   ...
   x = bx + ( tw - w ) / 2 + 2 * tw;
   y = by + bh - ( h + 20 ) + ( h + 20 - h ) / 2;
   equipment_renderOverlayColumn( x, y, h, p->outfit_structure, mover, wgt );
   mover -= array_size( p->outfit_structure );
   x -= tw;
   y = by + bh - ( h + 20 ) + ( h + 20 - h ) / 2;
   equipment_renderOverlayColumn( x, y, h, p->outfit_utility, mover, wgt );
   mover -= array_size( p->outfit_utility );
   x -= tw;
   y = by + bh - ( h + 20 ) + ( h + 20 - h ) / 2;
   equipment_renderOverlayColumn( x, y, h, p->outfit_weapon, mover, wgt );
   ...
   if ( o == NULL ) {
      ...
      if ( slot->sslot->exclusive && ( pos < (int)sizeof( alt ) ) )
         pos +=
            scnprintf( &alt[pos], sizeof( alt ) - pos, _( " [exclusive]" ) );
   ...
   /* Get text. */
   outfit_altText( alt, sizeof( alt ), o, p, slot );
...
static int equipment_mouseColumn( unsigned int wid, const SDL_Event *event,
                                  ...
                                  PilotOutfitSlot *os, Pilot *p, int selected,
                                  CstSlotWidget *wgt )
{
                               ...
                               PilotOutfitSlot *slot )
{
   ...
   if ( slot->outfit != NULL ) {
      ...
      if ( slot->u.ammo.deployed > 0 ) {
         int dockslot = -1;
         if ( !dialogue_YesNo( _( "Recall Fighters" ),
                               _( "This action will recall your deployed "
                                  "fighters. Is that OK?" ) ) ) {
            return 0;
         }
...
int equipment_canSwapPlayerShip( const char *shipname )
{
   ...
   if ( ship_isFlag( ps->p->ship, SHIP_NOPLAYER ) ) {
      land_errDialogueBuild( _( "You can not pilot the %s! The ship can only "
                                "be used as an escort." ),
                             shipname );
      return 0;
   }
   ...
   if ( ship_isFlag( player.p->ship, SHIP_NOESCORT ) ) {
      land_errDialogueBuild(
         _( "You can not swap ships and set %s as an escort!" ),
         player.p->name );
      return 0;
   }
   ...
   if ( ps->deployed )
      ...
      diff = pilot_cargoUsed( player.p ) - pilot_cargoFree( newship ) -
             ( pfleet_cargoFree() -
               pilot_cargoFree( player.p ) ); /* Has to fit all the cargo. */
   diff = MAX(
      diff, pilot_cargoUsedMission( player.p ) -
               pilot_cargoFree( newship ) ); /* Has to fit all mission cargo. */
   if ( diff > 0 ) { /* Current ship has too much cargo. */
      land_errDialogueBuild(
         n_( "You have %d tonne more cargo than the new ship can hold.",
 ...
 *    @param dpseps Whether or not to display damage per second and energy per second.
                         ...
                         int name )
{
   ...
   if ( name )
      ...
      l = 0;
   if ( dps > 0. )
      l += scnprintf( &buf[l], ( max_len - l ), _( "%.2f DPS [%.2f EPS]\n" ),
                      dps, eps );
...
static void equipment_toggleDeploy( unsigned int wid, const char *wgt )
{
   ...
   if ( strcmp( shipname, player.p->name ) != 0 ) {
      ...
      if ( state && ship_isFlag( ps->p->ship, SHIP_NOESCORT ) ) {
         dialogue_msg( _( "Invalid Escort" ),
                       _( "You can not set your ship '%s' as an escort!" ),
                       ps->p->name );
         return;
      }
...
static void equipment_genShipList( unsigned int wid )
{
   ...
   iconsize = 96;
   if ( !conf.big_icons ) {
      if ( toolkit_simImageArrayVisibleElements( sw, sh, iconsize, iconsize ) <
           nships )
         iconsize = 80;
      if ( toolkit_simImageArrayVisibleElements( sw, sh, iconsize, iconsize ) <
           nships )
   ...
   window_addImageArray( wid, 20, -40, sw, sh, EQUIPMENT_SHIPS, iconsize,
                         iconsize, cships, nships, equipment_updateShips,
                         equipment_rightClickShips, equipment_transChangeShip );
...
static void equipment_genOutfitList( unsigned int wid )
{
   ...
   /* Get dimensions. */
   equipment_getDim( wid, &w, &h, NULL, NULL, &ow, &oh, NULL, NULL, NULL, NULL,
                     NULL, NULL );
   ...
   if ( !widget_exists( wid, EQUIPMENT_OUTFIT_TAB ) ) {
      ...
      window_addButton( wid, ix + iw - 30, iy, 30, 30, "btnOutfitFilter", NULL,
                        equipment_outfitPopdown );
      window_buttonCustomRender( wid, "btnOutfitFilter",
   ...
   /* Get the outfits. */
   noutfits = player_getOutfitsFiltered( (const Outfit ***)&iar_outfits[active],
                                         tabfilters[active], filtertext );
   coutfits =
      outfits_imageArrayCells( (const Outfit **)iar_outfits[active], &noutfits,
                               ( p == NULL ) ? player.p : p, 0 );
   ...
   window_addImageArray( wid, x + 4, y + 3, iw, ih, EQUIPMENT_OUTFITS, iconsize,
                         iconsize, coutfits, noutfits, equipment_updateOutfits,
                         equipment_rightClickOutfits,
                         equipment_rightClickOutfits );

   toolkit_setImageArrayAccept( wid, EQUIPMENT_OUTFITS,
                                equipment_rightClickOutfits );
...
void equipment_updateShips( unsigned int wid, const char *str )
{
   ...
   equipment_getDim( wid, &ww, &wh, &sw, &sh, NULL, NULL, NULL, NULL, NULL,
                     NULL, &bw, &bh );
   ...
   /* update text */
   credits2str( buf_price, player_shipPrice( shipname, 0 ),
                2 ); /* sell price */
   ...
   acquired = ( ps->acquired )
                 ? ps->acquired
                 : _( "You do not remember how you acquired this ship." );
   ...
   if ( ship_mode == 0 ) {
      ...
      l += scnprintf( &buf[l], sizeof( buf ) - l, "\n" );
      l +=
         scnprintf( &buf[l], sizeof( buf ) - l, _( "#%c%s%.0f#0 %s" ),
      ...
      l += scnprintf(
         ...
         EQ_COMP( solid_maxspeed( &ship->solid, ship->speed, ship->accel ),
                  solid_maxspeed( &ship->solid, ship->ship->speed,
                                  ship->ship->accel ),
                  0 ),
         UNIT_SPEED );
      l += scnprintf(
         &buf[l], sizeof( buf ) - l, _( "\n#%c%s%.0f#0 %s" ),
      ...
      l += scnprintf( &buf[l], sizeof( buf ) - l, _( "%s %s" ),
                      num2strU( ship->ew_signature, 0 ), UNIT_DISTANCE );
      l += scnprintf( &buf[l], sizeof( buf ) - l, "\n%s %s\n",
                      num2strU( ship->ew_stealth, 0 ), UNIT_DISTANCE );
      l += scnprintf( &buf[l], sizeof( buf ) - l, _( "%.1f %s" ),
      ...
      l += scnprintf(
         &buf[l], sizeof( buf ) - l, _( " (#%c%s%s#0 %s)" ),
         EQ_COMP_I( ship->shield_regen, ship->ship->shield_regen, 0 ),
         num2strU( ship->shield_regen, 1 ), UNIT_POWER );
      l += scnprintf( &buf[l], sizeof( buf ) - l, "\n" );
      l += scnprintf( &buf[l], sizeof( buf ) - l, _( "#%c%s%s#0 %s" ),
                      EQ_COMP_I( ship->armour_max, ship->ship->armour, 0 ),
                      num2strU( ship->armour_max, 0 ), UNIT_ENERGY );
      l += scnprintf(
         &buf[l], sizeof( buf ) - l, _( " (#%c%s%s#0 %s)" ),
         EQ_COMP_I( ship->armour_regen, ship->ship->armour_regen, 0 ),
         num2strU( ship->armour_regen, 1 ), UNIT_POWER );
      l += scnprintf( &buf[l], sizeof( buf ) - l, "\n" );
      l += scnprintf( &buf[l], sizeof( buf ) - l, _( "#%c%s%s#0 %s" ),
                      EQ_COMP_I( ship->energy_max, ship->ship->energy, 0 ),
                      num2strU( ship->energy_max, 0 ), UNIT_ENERGY );
      l += scnprintf(
         &buf[l], sizeof( buf ) - l, _( " (#%c%s%s#0 %s)" ),
         EQ_COMP_I( ship->energy_regen, ship->ship->energy_regen, 0 ),
         num2strU( ship->energy_regen, 1 ), UNIT_POWER );
      l += scnprintf( &buf[l], sizeof( buf ) - l, "\n" );
      /* Misc. */
      l += scnprintf( &buf[l], sizeof( buf ) - l, _( "%s / #%c%s%s#0 %s" ),
                      scargo, EQ_COMP_I( cargo, ship->ship->cap_cargo, 0 ),
                      num2strU( cargo, 0 ), UNIT_MASS );
      l += scnprintf( &buf[l], sizeof( buf ) - l, "\n" );
      l += scnprintf( &buf[l], sizeof( buf ) - l, _( "%s %s (%d %s)" ),
                      num2strU( ship->fuel_max, 0 ), UNIT_UNIT, jumps,
                      n_( "jump", "jumps", jumps ) );
      l += scnprintf( &buf[l], sizeof( buf ) - l, "\n" );
      /*l +=*/scnprintf( &buf[l], sizeof( buf ) - l, "\n#%c%s#0",
                         spaceworthy ? '0' : 'r', errorReport );
   } else {
      int destroyed = 0;
      ...
      l += scnprintf( &buf[l], sizeof( buf ) - l, "\n%s",
                      num2strU( ps->landed_times, 0 ) );
      l += scnprintf( &buf[l], sizeof( buf ) - l, "\n" );
      l += scnprintf( &buf[l], sizeof( buf ) - l, _( "%s %s" ),
                      num2strU( ps->dmg_done_shield + ps->dmg_done_armour, 0 ),
                      UNIT_ENERGY );
      l += scnprintf( &buf[l], sizeof( buf ) - l, "\n" );
      l +=
         scnprintf( &buf[l], sizeof( buf ) - l, _( "%s %s" ),
                    num2strU( ps->dmg_taken_shield + ps->dmg_taken_armour, 0 ),
                    UNIT_ENERGY );
      /*l +=*/scnprintf( &buf[l], sizeof( buf ) - l, "\n%s",
                         num2strU( destroyed, 0 ) );
   }
   ...
   if ( ship_mode != 0 ) {
      ...
      window_addImageArray( wid, tx, ty, ww - x - 128 - 30 - 10,
                            ty - 20 + wh - bh - 30, "iarIntrinsic", 64, 64,
                            cells, ncells, NULL, NULL, NULL );
...
static void equipment_outfitPopdown( unsigned int wid, const char *str )
{
   ...
   window_addList( wid, x + w, y - 120 + h, 350, 120, name, modelist, n,
                   equipment_outfitMode, equipment_outfitPopdownSelect,
...
static void equipment_unequipShip( unsigned int wid, const char *str )
{
   ...
   if ( pilot_cargoUsed( ship ) > ship->ship->cap_cargo ) {
      dialogue_alert( _( "You can't unequip your ship when you have more cargo "
                         "than it can hold without modifications!" ) );
      return;
   }
   if ( dialogue_YesNo( _( "Unequip Ship" ), /* confirm */
                        _( "Are you sure you want to remove all equipment from "
                           "your ship?" ) ) == 0 )
      return;
...
static void equipment_autoequipShip( unsigned int wid, const char *str )
{
   ...
   if ( autoequip_env == NULL ) {
      ...
      /* New env. */
      autoequip_env = nlua_newEnv( file );
   ...
   if ( !lua_isfunction( naevL, -1 ) ) {
      WARN( _( "'%s' doesn't have valid required 'autoequip' function!" ),
            file );
      lua_pop( naevL, 1 );
      goto autoequip_cleanup;
   }
...
static void equipment_sellShip( unsigned int wid, const char *str )
{
   ...
   if ( !dialogue_YesNo(
           _( "Sell Ship" ),
           _( "Are you sure you want to sell your ship %s for %s?" ), shipname,
           buf ) )
      return;
   ...
   /* Display widget. */
   dialogue_msg( _( "Ship Sold" ), _( "You have sold your ship %s for %s." ),
                 name, buf );
...
void equipment_slotSelect( CstSlotWidget *wgt, PlayerShip_t *p )
{
   ...
   for ( int i = 0; i < array_size( p->p->outfits ); i++ ) {
      Outfit *o = (Outfit *)p->p->outfits[i]->outfit;
      if ( ( o != NULL ) && !outfit_gfxStoreLoaded( o ) ) {
