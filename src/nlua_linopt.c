...
#define LINOPT_MAX_TM                                                          \
   1000 /**< Maximum time to optimize (in ms). Applied to linear relaxation    \
           and MIP independently. */
 ...
 *    @luatparam[opt=false] boolean glpk_format Whether the program is in GLPK
 * format instead of MPS format.
 *    @luatparam[opt=false] boolean maximize Whether to maximize (rather than
 * minimize) the function.
 ...
 *    @luatparam[opt=false] boolean glpk_format Whether to write the program in
 * GLPK format instead of MPS format.
