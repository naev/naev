...
#define MAX_EFFECT_RADIUS                                                      \
   4 /**< Maximum pixel distance from glyph to outline/shadow/etc. */
...
#define DEFAULT_TEXTURE_SIZE                                                   \
   1024             /**< Default size of texture caches for glyphs. */
...
static FT_UInt
   prev_glyph_index; /**< Index of last character drawn (for kerning). */
...
/**
 * @brief Stores a font file. May be referenced by multiple glFonts for size or
 * fallback reasons.
 */
...
static glFontStash *avail_fonts =
   NULL; /**< These are pointed to by the font struct exposed in font.h. */
...
static const glColour *font_lastCol =
   NULL; /**< Stores last colour used (activated by FONT_COLOUR_CODE). */
...
static int    gl_fontstashAddFallback( glFontStash *stsh, const char *fname,
                                       unsigned int h );
static size_t font_limitSize( glFontStash *stsh, int *width, const char *text,
                              const int max );
...
/* Render.
 * TODO this should be changed to be more like font-stash
 * (https://github.com/akrinke/Font-Stash) In particular, instead of writing
 * char by char, they should be batched up by textures and rendered when
 * gl_fontRenderEnd() is called, saving lots of opengl calls.
 */
static void gl_fontRenderStart( const glFontStash *stsh, double x, double y,
                                const glColour *c, double outlineR );
static void gl_fontRenderStartH( const glFontStash *stsh, const mat4 *H,
                                 const glColour *c, double outlineR );
static int  gl_fontRenderGlyph( glFontStash *stsh, uint32_t ch,
                                const glColour *c, int state );
static void gl_fontRenderEnd( void );
/* Fussy layout concerns. */
static void gl_fontKernStart( void );
static int  gl_fontKernGlyph( glFontStash *stsh, uint32_t ch,
                              glFontGlyph *glyph );
                               ...
                               glFontGlyph *glyph )
{
   ...
   for ( int i = 0; i < array_size( stsh->tex ); i++ ) {
      for ( int j = 0; j < MAX_ROWS; j++ ) {
         ...
         if ( j == 0 ) {
            assert( ch->h <=
                    stsh->th ); /* Would be ridiculously large character... */
            r->h = ch->h;
            tex  = &stsh->tex[i];
            gr   = r;
            break;
         ...
         break; /* Have to break here because either we added a new row or
                   texture is full. */
      }
      if ( gr != NULL )
         break;
   ...
   if ( gr == NULL ) {
      ...
      /* Initialize size. */
      glTexImage2D( GL_TEXTURE_2D, 0, GL_RED, stsh->tw, stsh->th, 0, GL_RED,
                    GL_UNSIGNED_BYTE, NULL );

      /* Check for errors. */
      gl_checkErr();

      /* Create a new entry at the beginning of the first row with our target
       * height. */
      gr    = &tex->rows[0];
      gr->h = ch->h;
   }
   ...
   if ( stsh->nvbo > stsh->mvbo ) {
      stsh->mvbo *= 2;
      stsh->vbo_tex_data =
         realloc( stsh->vbo_tex_data, 8 * stsh->mvbo * sizeof( GLfloat ) );
      stsh->vbo_vert_data =
         realloc( stsh->vbo_vert_data, 8 * stsh->mvbo * sizeof( GLshort ) );
   }
   n        = 8 * stsh->nvbo;
   ...
   gl_vboActivateAttribOffset( stsh->vbo_vert, shaders.font.vertex, 0, 2,
                               GL_SHORT, 0 );
   gl_vboActivateAttribOffset( stsh->vbo_tex, shaders.font.tex_coord, 0, 2,
                               GL_FLOAT, 0 );
...
/**
 * @brief Stores the colour information from a piece of text limited to max
 * characters.
...
typedef struct _linepos_t_ {
   ...
   GLfloat
      w; /**< Current width (line start to left side of current character) */
} _linepos_t;
...
int gl_printLineIteratorNext( glPrintLineIterator *iter )
{
   ...
   while ( pos.ch != '\0' ) {
      ...
      if ( can_break && iswspace( pos.ch ) ) {
         ...
         /* IMPORTANT: when eating a space, we can't backtrack to a previous
          * position, because there might be a skipped font markup sequence in
          * between. */
...
static uint32_t font_nextChar( const char *s, size_t *i )
{
   uint32_t ch = s[*i]; /* To be corrected: the character starting at byte *i.
                           Whether it's zero or not is already correct. */
   while ( ch != 0 ) {
      ch = u8_nextchar( s, i );
      if ( ch != FONT_COLOUR_CODE )
         return ch;
 ...
 */
int gl_printMidRaw( const glFont *ft_font, int width, double x, double y,
                    const glColour *c, double outlineR, const char *text )
{
   int      n, s;
   size_t   ret, i;
                  ...
                  const char *fmt, ... )
{
   ...
   return gl_printTextRaw( ft_font, width, height, bx, by, line_height, c, -1.,
                           text );
}
 ...
 */
int gl_printHeightRaw( const glFont *ft_font, const int width,
                       const char *text )
{
   glPrintLineIterator iter;
   int                 line_height;
   ...
   for ( int i = 0; i < len; i++ ) {
      ...
      if ( glyph_index == 0 ) {
         if ( i < len - 1 )
            ...
            WARN( _( "Font '%s' unicode character '%#x' not found in font! "
                     "Using missing glyph." ),
                  ft->file->name, ch );
      ...
      /* Load the glyph. */
      if ( FT_Load_Glyph( ft->face, glyph_index,
...
/**
 * @brief Return the signed advance (same units as adv_x) ahead of the current
 * char.
 */
static int gl_fontKernGlyph( glFontStash *stsh, uint32_t ch,
                             glFontGlyph *glyph )
{
   ...
   if ( prev_glyph_index && prev_glyph_ft_index == glyph->ft_index ) {
      FT_Vector kerning;
      FT_Get_Kerning( ft_face, prev_glyph_index, ft_glyph_index,
                 ...
                 const char *prefix, unsigned int flags )
{
   ...
   if ( !( flags & FONT_FLAG_DONTREUSE ) ) {
      for ( int i = 0; i < array_size( avail_fonts ); i++ ) {
         stsh = &avail_fonts[i];
         if ( stsh->fname == NULL ) {
            /* This glFontStash must have been zeroed by gl_freeFont after its
                                    ...
                                    unsigned int h )
{
   ...
   if ( FT_New_Memory_Face( font_library, ft.file->data, ft.file->datasize, 0,
                            &ft.face ) ) {
   ...
   if ( FT_IS_SCALABLE( ft.face ) ) {
      FT_Matrix scale;
      if ( FT_Set_Char_Size( ft.face, 0, /* Same as width. */
                             h * 64, 96, /* Create at 96 DPI */
                             96 ) )      /* Create at 96 DPI */
