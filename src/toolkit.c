 ...
 */
const glColour _toolkit_colLight = { .r=0.17953507505959196, .g=0.17953507505959196, .b=0.19599417930148813, .a=1.0 };     /**< Light outline colour. */
const glColour *toolkit_colLight = &_toolkit_colLight;
const glColour _toolkit_col      = { .r=0.02842782824665854, .g=0.02842782824665854, .b=0.030666681641938164, .a=1.0 };    /**< Normal outline colour. */
const glColour *toolkit_col      = &_toolkit_col;
const glColour _toolkit_colDark  = { .r=0.0035924493743964936, .g=0.0035924493743964936, .b=0.00375840377954798, .a=1.0 }; /**< Dark outline colour. */
const glColour *toolkit_colDark  = &_toolkit_colDark;

/*
 * Tab colours
 */
const glColour _tab_active     = { .r=0.02842782824665854, .g=0.02842782824665854, .b=0.030666681641938164, .a=1.0 };   /**< Light outline colour. */
const glColour *tab_active     = &_tab_active;
const glColour _tab_activeB    = { .r=0.008848905697420069, .g=0.008848905697420069, .b=0.009413232170634472, .a=1.0 }; /**< Light outline colour. */
const glColour *tab_activeB    = &_tab_activeB;
const glColour _tab_inactive   = { .r=0.017014804602423293, .g=0.017014804602423293, .b=0.01825750718659873, .a=1.0 };  /**< Normal outline colour. */
const glColour *tab_inactive   = &_tab_inactive;
const glColour _tab_inactiveB  = { .r=0.008848905697420069, .g=0.008848905697420069, .b=0.009413232170634472, .a=1.0 }; /**< Normal outline colour. */
...
{
   gl_renderRectEmpty( x - b, y - b, w + 2 * b, h + 2 * b, c );
}
/**
 ...
 *    @param c Colour.
 */
void toolkit_drawRectHalf( int x, int y, int w, int h, const glColour *c )
{
   gl_renderRectHalf( x, y, w, h, c );
}
...
void toolkit_drawAltText( int bx, int by, const char *alt )
{
   ...
   c2.b = cGrey10.b;
   c2.a = 0.7;
   toolkit_drawRect( x + 1, y + 1, w + 18, h + 18, &c2 );
   toolkit_drawRect( x, y, w + 18, h + 18, &c );
...
static void window_renderBorder( const Window *w )
{
   ...
   if ( window_isFlag( w, WINDOW_FULLSCREEN ) ) {
      /* Background. */
      toolkit_drawRect( x, y, w->w, w->h, toolkit_col );
      /* Name. */
   ...
   toolkit_drawRect( x, y, w->w, w->h, toolkit_col );
   toolkit_drawOutlineThick( x, y, w->w, w->h, 1, 2, toolkit_colDark );
   ...
   for ( Widget *wgt = w->widgets; wgt != NULL; wgt = wgt->next ) {
      ...
      if ( wgt->id == w->focus ) {
         ...
         toolkit_drawOutlineThick(
            ...
            ( wgt->type == WIDGET_BUTTON ? &cGrey70 : &cGrey30 ) );
      }
   }
}
...
void toolkit_drawScrollbar( int x, int y, int w, int h, double pos )
{
   ...
   toolkit_drawRect( x, y, w, h, &cGrey10 );

   /* Bar itself. */
   sy = y + ( h - 30. ) * ( 1. - pos );
   toolkit_drawRect( x, sy, w, 30., toolkit_colLight );
   toolkit_drawOutline( x + 1, sy, w - 2, 30., 0., toolkit_colDark );
}
