...
impl NebulaData {
   ...
   pub fn render(&self, ctx: &renderer::Context) -> Result<()> {
      ...
      unsafe {
         let screen = std::num::NonZero::new(naevc::gl_screen.current_fbo).map(NativeFramebuffer);
   ...
   pub fn setup(
      ...
      unsafe {
         ...
         // Trails
         let col = Srgb::from_color(Hsv::new(360.0 * hue, 0.7 * saturation, value)).into_linear();
