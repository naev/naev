...
### Gameplay
  ...
  * NPCs now have skill levels that can affect their targeting and fighting
    ability
  * Bioships have had their skills and modifiers slightly reworked to be better
    balanced
...
### Quality of Life
  ...
  * Support for automatically restarting Naev when changing options that
    require it
  * Systems with uninhabited but landable space objects will have smaller inner
    circles
...
### Contents
  ...
  * Reworked escort missions so that escorts follow the player and improved how
    it interacts with autonav
...
### Engine
  ...
  * Camera is isometric-ish instead of using top-view game play (can be
    modified by plugins)
  * Engine supports modifying some important constants such as those related to
    the physics model for use in plugins
...
## 0.12.0-beta.2
  ...
  * Editor now shows dialogues on errors and allows user to choose directory to
    save data to
  * Minor typo fixes and translation updates
...
### Engine
  ...
  * Lazy load ship and outfit graphics to reduce memory usage and decrease load
    time
...
## 0.11.1
  ...
  * `kidnapped/traffic_00`: fixed formatting string telling the player what
    system to go to
  * `kidnapped/traffic_01`: ship should spawn if taking off in the same system,
    not just jumping in
...
## 0.11.0
  ...
  * `nelly01`: fixed giving the player twice as much money as needed for an ion
    cannon
...
### Gameplay Changes
  ...
  * Pirate bribes cost more depending on your fleet and are based on points not
    mass
...
### Quality of Life
  ...
  * Independent patrol and bounty missions can be completed on more planets and
    stations
  ...
  * Changed the faction standing caps to allow the player to get all ships when
    maxed out. Will be decreased in the future as missions are added
  * Slot icons to make it more clear what special slots a ship has
  * System markers and autonav TARGET marker will try to not overlap with jumps
    and spob names
...
## 0.9.0
  ...
  * A busy volunteer proofreader kept editing almost as quickly as we could add
    errores
...
### New mechanics
  ...
  * Changed ship classification, removing rare classes while adding Interceptor
...
### Engine
  ...
  * Restored macOS support. (Catalina users will have to bypass Gatekeeper: See
    https://codeberg.org/naev/naev/wiki/FAQ for details.)
  * Fixed a crash-loop when the "saves" folder gets populated by Steam data (or
    other files) and no Naev save files
