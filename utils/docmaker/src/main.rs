...
fn ldoc(blocks: &[String]) -> Result<String> {
    ...
    let re_syntax = Regex::new(r"@(\w+)")?;

    Ok(blocks
        .iter()
        .map(|body| {
            let body = re_syntax.replace_all(body, |caps: &Captures| {
                map.get(&caps[1]).copied().unwrap_or(caps.get(0).unwrap().as_str()).to_owned()
            });
...
fn luacheck(blocks: &[String]) -> Result<(String,String)> {
    let modname = {
        ...
        if mods.len() > 1 {
            anyhow::bail!(format!("Multiple modules in one document not supported yet! Found: {}", mods.join(", ")));
        } else if mods.len() == 0 {
...
fn main() -> Result<()> {
    ...
    let output = match &args.command {
        ...
        Mode::Luacheck(lc) => {
            ...
            output.push_str("-- THIS FILE IS AUTOMATICALLY GENERATED\n");
            output.push_str("-- ALL LOCAL CHANGES WILL BE LOST ON REGENERATION, SO DO NOT MODIFY PLEASE\n");
