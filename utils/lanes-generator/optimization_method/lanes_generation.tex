...
\begin{table}[ht]
	...
	\begin{tabular}{|c|c|c|c|c|} \hline
		Domain & $u$ & $f$ & k & Remark \\ \hline
		Economy & Quantity of goods & Flux of goods & ? & \\ \hline
		Traffic modelling & Population & Traffic & ? & It's our case \\ \hline
		Thermic & Temperature & Heat flux & Thermal conductivity & \\ \hline
		...
		Mechanics & Displacement & Stress & Stiffness & $u$ is a vector \\ \hline
	\end{tabular}
...
This means that in our particular application, the cost-function $\varphi$ does not depend on $\theta$ directly. In a matricial notation, we introduce the sparse difference matrix $Q$ of size \textcolor{red}{??}:
...
\begin{equation}
\psi(\theta,\tilde{U},\Lambda) = \dfrac{1}{2} \|Q\tilde{U}P_p^T\|_f^2 + \Lambda \cdot \left( K(\theta)\tilde{U} - \tilde{F} \right)
\end{equation}
...
An idea could be to use a gradient-based minimization algorithm (BFGS or steepest descent) to minimize $\phi$ under a few additional constraints, with respect to $\theta \in [0;1]$, and to use a regularization function, or any other procedure that tends to impose that $\theta$ is $0$ or $1$. However, this approach is quite complex and not straightforwardly compatible with the fact that there are different factions, that minimize different cost-functions.
 ...
 In this report, an other approach is proposed. It consists in activating at each iteration one lane per system and per faction. This lane is the one with the strongest gradient (in absolute value). It is presented at algorithm \ref{alg:lanes}.
...
\begin{equation}
\varphi = \dfrac{1}{2} \sum_{i,l} \sum_{j,k} q_{ij} \tilde{u}_{jk} p_{lk}
\end{equation}
...
\section*{Annex C: Evaluation of the gradient}
