...
"$SCRIPT_DIR"/sys2dot.py $COL "$DST"/*.xml -k | neato -n2 -Tpng 2>/dev/null > before.png
echo "gen after graph " >&2
"$SCRIPT_DIR"/sys2dot.py "$DST"/*.xml | tee before.dot | neato 2>/dev/null | tee after.dot | neato -n2 -Tpng 2>/dev/null > after.png
