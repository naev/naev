...
#define TO( num ) ( FROM( ( num ) + 1 ) - 1 )
   ...
   for (num = 0; num < PROCESSES - 1; num++) {
      ...
      if (fork() == 0) {
         for (int i = FROM( num ); i <= TO( num ); i++) {
            const float per =
               100 * ( i - FROM( num ) ) / ( TO( num ) - FROM( num ) );
            if (percent[num] != per) {
               int count    = 1;
               percent[num] = per;
            ...
            for (int j = minj; j <= maxj; j++) {
               ...
               maps[num][( i - FROM( num ) ) * ( maxj - minj + 1 ) +
                         ( j - minj )] = f;
            }
         }
         msync( maps[num],
   ...
   for (num = 0; num < PROCESSES - 1; num++) {
      memcpy( map + ( FROM( num ) - mini ) * ( maxj - minj + 1 ), maps[num],
              ( TO( num ) - FROM( num ) + 1 ) * ( maxj - minj + 1 ) *
                 sizeof( float ) );
      munmap( maps[num], ( TO( num ) - FROM( num ) + 1 ) * ( maxj - minj + 1 ) *
                            sizeof( float ) );
   }
