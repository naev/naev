...
Although it is usually preferrable to [use 3D graphics](./3d.md), you can use 2D graphics for a more classic look instead.
...
Assuming `GFX` is the graphics name and `DIR` is the directory name (up to first underscore in `GFX`), we get (extension is autodetected if omitted):
