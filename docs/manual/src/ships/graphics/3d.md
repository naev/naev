...
You can use 3D models to represent ships in Naev by exporting them to GLTF 2, which is [supported by Blender](https://docs.blender.org/manual/en/latest/addons/import_export/scene_gltf2.html) and most other major 3D modelling software.
...
The ship should be pointing in the `-Y` direction, and the scale doesn't matter, since it will get rescaled on loading.
...
| ------------- | ---- | ----- |
| `NAEV_weapon_mount` | String | Index of the mount point starting at 0. They should be consecutive: do not leave a number out! |
| `NAEV_trail_generator` | String | Name of the trail type. Has to match a name of a trail defined in `trails/`. |
...
You can have Naev autodetect image formats by not using the file extensions.
This allows Naev to use `.avif` files even though they are not specified in the official specification.

You can automate the process from python using the following script, which also uses [glTransform](https://gltf-transform.dev/) to optimize the model and such.
```py
import os
import subprocess
import bpy
