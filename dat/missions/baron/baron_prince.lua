...
function accept()
   ...
   if tk.yesno(_("His Baronship remembers you"), _([[As you approach the stranger, he extends his hand in greeting. He introduces himself as an associate of Baron Sauterfeldt, the man you helped to "acquire" a holopainting not too long ago.
    ...
    You decide to keep your opinion of the aristocrat to yourself. Instead you inquire as to what the man wants from you this time. "To tell the truth, I don't actually know," the man says. "The Baron wants you to meet him so he can brief you in person. You will find his ship in the %s system. Shall I inform his lordship that you will be paying him a visit?"]]):format(baronsys:name())) then
      misn.accept()
      tk.msg(_("At your beck and call"), _([["Splendid. Please go see his lordship at the earliest opportunity. He doesn't like to be kept waiting. I will send word that you will be coming, so contact the Pinnacle when you arrive at %s, and they will allow you to board."]]):format(baronsys:name()))
...
function board()
   if stage == 1 then
    ...
    "I should warn you, %s. Some of my, ah, colleagues have also set their sights on this item, and so you can expect their henchmen to try to take it off you. I trust you are equipped to defend yourself against their despicable sort."]]):format(mangle(player.name()), flintplanet:name(), flintsys:name(), mangle(player.name())))
      ...
      idle()
   elseif stage == 2 then
      tk.msg(_("What are you doing here?"), _([[You have not yet collected and identified the genuine artifact. Buy the artifacts from the sellers and visit Flintley on %s (%s) to identify the real one.]]):format(flintplanet, flintsys))
      player.unboard()
      pinnacle:setHealth(100,100)
      idle()
   elseif stage == 3 then
      tk.msg(_("The Baron has his prize"), _([[Baron Dovai Sauterfeldt turns the skate-board over in his hands, inspecting every nick, every scratch on the surface. His eyes are gleaming with delight.
...
function flintley()
   ...
   if flintleyfirst then
      flintleyfirst = false
      tk.msg(_("Flintley, at your service"), _([[You approach the nervous-looking man and inquire if he is Flintley, the historian in Baron Sauterfeldt's employ.
    ...
    Flintley sighs. "Well, that's that. Come to me with any artifacts you manage to procure, and I'll analyze them to the best of my ability."]]):format(player.name()))
   ...
   if artifactB ~= nil then
      if rnd.rnd(1, 3 - artifactsfound) == 1 then
         ...
         tk.msg(_("This is not the artifact you're looking for"), _([[You hand Flintley the artifact you procured on %s. He examines it for a few moments, then enters a few queries in the info terminal in his table. Once he has found what he was looking for, he heaves a sigh. "I'm sorry, %s. It seems you've been had. What you've got here is little more than a trinket. It's a piece of 'art' created by a third-rank sculptress named Biena Gharibri who lives on Lapra. She's not very talented, I'm afraid. Her creations have been called 'worse than Dvaered opera' by a leading art critic. I really don't think you want to present his lordship with this."
    You promptly decide to dispose of the thing, unwilling to carry it around with you a moment longer than necessary.]]):format(artifactplanetB:name(), player.name()))
         artifactsfound = artifactsfound + 1
      end
...
function seller()
   if planet.cur() == artifactplanetA then
      if tk.choice(_("A suspicious salesman"), _([["Hello there," the guy says to you when you approach. "Can I interest you in this bona fide relic from an ancient past? Unlike all those scammers out there, I offer you the real deal, no fakes here!"]]), _("Buy the artifact (%s)"):format(fmt.credits(reward * 0.15)), _("Don't buy the artifact")) == 1 then
   ...
   elseif planet.cur() == artifactplanetB then
      if tk.choice(_("A suspicious salesman"), _([[The man grins at you. "Ah, I can tell you have the eye of a connoisseur! I deal only in the finest, counterfeit-free antiques. If you're smart, and I can see that you are, you won't trust all those opportunists who will try to sell you fakes! How about it?"]]), _("Buy the artifact (%s)"):format(fmt.credits(reward * 0.15)), _("Don't buy the artifact")) == 1 then
         if player.credits() >= reward * 0.15 then
            misn.npcRm(sellnpc)
            player.pay( reward * 0.15 )
   ...
   elseif planet.cur() == artifactplanetC then
      if tk.choice(_("A suspicious salesman"), _([[The woman beckons you over to the bar. "Listen, friend. I have here a unique, extremely rare remnant of prehistoric times. This is the genuine article, trust me on that. One hundred per cent legit! And you wouldn't want to spend good credits on a fake, right?"]]), _("Buy the artifact (%s)"):format(fmt.credits(reward * 0.15)), _("Don't buy the artifact")) == 1 then
         if player.credits() >= reward * 0.15 then
            misn.npcRm(sellnpc)
            player.pay( reward * 0.15 )
