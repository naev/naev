...
function accept()
   if var.peek("achack02repeat") then
    ...
    "Pride, I suppose. I'm a military officer. It wouldn't look too tactful if I asked for personal protection when the navy is already stretched thin out there, trying to protect our civilians from pirates and other criminals every day. Besides, my conflict with Harja is a personal matter. I feel I should resolve this with my own resources." She gives you a tired smile. "That being said, I wouldn't say no to a helping hand."]]), {player=player.name()}))
   end
...
function enter()
   ...
   if mem.warnFuel then
      if player.pilot():stats().fuel < 7*player.pilot():stats().fuel_consumption then
         tk.msg(_("Fuel Warning"),_("You don't have enough fuel for making 7 jumps. You'll have to buy some from civilian ships on the way."))
      end
      mem.warnFuel = false
   end
...
function ambushSet(ships, location)
   ...
   hook.timer(0.5, "proximity", {anchor = ambush[1], radius = 3000, funcname = "ambushActivate", focus = joanne})
end
