...
"The plan is to send an advanced convoy with guards to do the route in an attempt to confuse possible enemies.  You will then go in and actually do the delivery by yourself.  This way we shouldn't arouse suspicion.  You are to report here when you finish delivery and you'll be paid %d credits."]]
   ...
   text[6] = [[You arrive to %s and report to Commander Soldner.  He greets you and starts talking, "I heard you encountered resistance.   At least you managed to deliver the package.  Great work there.  If you're still interested in more work meet me up in the bar in a bit, need to finish some paperwork first."]]
   -- Errors
   errtitle = {}
...
function land ()
   ...
   if landed == pickup and misn_stage == 0 then
      ...
      -- Update mission
      package = player.addCargo("Packages", 3)
      misn_stage = 1
      jumped = 0
   ...
   elseif landed == dest and misn_stage == 1 then
      if player.rmCargo(package) then
