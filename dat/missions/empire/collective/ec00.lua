...
"Hello, {player}; we have a reconnaissance mission you might be interested in. Commander Soldner said you'd make a good candidate for the mission. You up for the challenge?"]]), {player=player.name()}))
   ...
   dimitri(fmt.f(_([["They completely wiped out all human life on {planet} and the other worlds they hit. We managed to stop them here, in {sys}, and constructed this base. Since then, it's been more or less a stalemate."]]), {planet=_("Eiroik"), sys=misn_base_sys}))
   ...
   dimitri(fmt.f(_([["You are to locate the scout and report back to {pnt} in the {sys} system. It seems like the Collective is planning something, and we want to follow their game a little more closely."]]), {pnt=misn_base, sys=misn_base_sys}))
