...
Faction: {fct}
Cost: {credits}]]), {fct=fac, credits=fmt.credits(mem.fine)}))
    ...
    if hookfac == fac then
        if delta >= 0 then
            ...
            if mem.rep >= 0 then
                ...
                fac:modPlayerRaw(-delta + mem.rep)
            ...
            mem.excess = mem.excess + delta
            if mem.excess < 0 or fac:playerStanding() < 0 then
                abort()
            end
        end
    end
