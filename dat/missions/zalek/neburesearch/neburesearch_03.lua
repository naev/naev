...
lab_coat_title = _("Lab Coat Shop")
...
talk_prepared_text = _([[%s
    You start reciting the text you got from the student. It turned out learning your text was the right choice. Some of the occurring terms are difficult to pronounce correctly.]])
talk_glasses_text = _([[%s
    You start reciting the text you got from the student. With the text being displayed on your glasses you don't have to look down on a paper to read the text, but some of the occurring terms are difficult to pronounce correctly.]])
...
choice_lab_coat = _("Buy a lab coat")
...
function create()
    ...
    -- Spaceport bar stuff
    misn.setNPC(_("Robert"), "zalek/unique/student.png", bar_desc)
end
...
function enter_spaceport()
    ...
    if has_lab_coat then
        ...
        if has_glasses then
            c = tk.choice("", _("You wonder what to do..."), choice_return, choice_institute, choice_lab_coat)
        else
            c = tk.choice("", _("You wonder what to do..."), choice_return, choice_institute, choice_lab_coat, choice_electronics)
        end
    end
    if c == 1 then
        tk.msg("", return_text)
...
function start_talk()
    ...
    local text1
    if has_lab_coat then
        text1 = has_lab_coat_text
        faction.modPlayerSingle("Za'lek", 1)
    else
        text1 = no_lab_coat_text
    end
