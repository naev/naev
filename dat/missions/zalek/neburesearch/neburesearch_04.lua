...
function accept()
    ...
    misn.accept()
    osd_msg[1] = osd_fly_to:format(_(dest_planet:name()), _(dest_sys:name()))
    osd_msg[2] = osd_return:format(_(homeworld:name()), _(homeworld_sys:name()))
...
function land()
    ...
    if landed == dest_planet then
        if stage == 0 then
            ...
            tk.msg(setback_title, first_planet_text:format(_(planet_name), _(dest_planet:name()), _(dest_sys:name())))
        ...
        elseif stage == 2 then
            ...
            tk.msg(setback_title, second_planet_text:format(_(planet_name), _(dest_planet:name()), _(dest_sys:name())))
            misn.markerMove(misn_marker, dest_sys)
            osd_msg[1] = osd_fly_to:format(_(dest_planet:name()), _(dest_sys:name()))
            osd_msg[2] = osd_return:format(_(homeworld:name()), _(homeworld_sys:name()))
            misn.osdCreate(osd_title, osd_msg)
...
function cannotLand()
    ...
    tk.msg(ruadan_title, ruadan_text:format(_(planet_name), _(planet_name), _(dest_planet:name())))
    osd_msg[1] = osd_fly_to:format(_(dest_planet:name()), _(dest_sys:name()))
    osd_msg[2] = osd_return:format(_(homeworld:name()), _(homeworld_sys:name()))
    misn.osdCreate(osd_title, osd_msg)
end
