...
function land()
   if mem.misn_stage == STAGE_RETURN_JORLA and spob.cur() == homeworld then
      ...
      vn.na(fmt.f(_("He gives you a credit chip worth {credits} and heads off. The money is nice, but not worth as much as the insight that working for the Za'lek will be dangerous and tiresome."),{credits="#g"..fmt.credits(credits).."#0"}))
      vn.done()
      vn.run()
...
function startProblems()
   ...
   local _dens, volatility = system.cur():nebula()
   local armour = player.pilot():armour(true)
   local time_to_die = math.min( armour / (volatility * 3.), 13 )
   ps:setNoDeath(true)
   hook.timer(0.35 * time_to_die, "noticeProblems")
   hook.timer(0.95 * time_to_die , "stopProblems")
end

function drainShields()
   local ps = player.pilot()
   local armour = ps:armour()
