...
function land()
   if mem.misn_stage == STAGE_RETURN_JORLA and spob.cur() == homeworld then
      ...
      vn.na(fmt.f(_("He gives you a credit chip worth #g{credits}#0 and heads off. The money is nice, but not worth as much as the insight that working for the Za'lek will be dangerous and tiresome."),{credits=fmt.credits(credits)}))
      vn.done()
      vn.run()
