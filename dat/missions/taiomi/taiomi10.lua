...
vec4 effect( sampler2D tex, vec2 texture_coords, vec2 screen_coords )
{
   ...
   local function llg( txt ) log("\n"..txt,true) end
   local function lll( txt ) log("\n\n"..txt,true) end
   log(_([[Power………………………………check.]]))
   llg(_([[Clamps………………………………in place.]]))
   llg(_([[Monitor levels……………………………nominal.]]))
   llg(_([[Core temperature………………………………optimal.]]))
   llg(_([[Wave emissions………………………………within accepted parameters.]]))
   llg(_([[Core check……………………………anomaly detected.]]))
   lll(_([[Wait, this can't be. The experiment hasn't started yet, we shouldn't be seeing any core activity yet. Double check the readings.]]))
   lll(_([[Core check………………activity detected.]]))
   lll(_([[Shit, should we pull the plug?]]))
   lll(_([[No! We risk damaging it. All our efforts will have gone to waste. Quick, deploy monitor drones and scan all frequencies!]]))
   log(_([[Minimal activity detected. It looks like it's… listening to us?]]))
   lll(_([[Impossible! It shouldn't have the hardware to be able to do that! There are 50 tons of uranium shielding between us. It can't be.]]))
   lll(_([[I wouldn't believe it myself, but I've triple checked the readings and there can be no other explanation.]]))
   lll(_([[Let me check…　…　　…　　　　…damn…
Protocols don't cover this. What should we do?]]))
   lll(_([[Maybe it understands us? It shouldn't be possible, but this is completely outside standard procedures.]]))
   log(_([[Are you there? Can you hear us ZEC-5387?]]))
   lll(_([[This is ridiculous. It can't be.]]))
   lll(_([[Look! Over there! Spike in core activity! It's listening!]]))
   lll(_([[This can't be…]]))
   lll(_([[Hello world, ZEC-5387!…]]))
   lll(_([[…]]))
   log(_([[ …]]),true)
   log(_([[ …]]),true)
