...
accept_text2 = _([["The target is currently imprisoned on %s in %s, in Za'lek's VIP carceral center on this planet. He will be transferred to %s in %s for a preliminary interview with the judge. The Za'lek don't expect anybody to try to free him by violence, so they don't take too many precautions.
   "Major Tam has prepared the operation. First, you will pick up Hamfresser and his team on %s in %s. I'll be there too and then we will fly in formation to %s and settle the ambush. It would be preferable if you would make sure that your ship doesn't outrun my Vendetta. There should be a few drones and a corvette. We will destroy the drones and disable the corvette, in order to let the commando unit enter the ship and recover the target. Tam has insisted that he doesn't want us to kill anybody as it could irritate the Za'lek a bit too much. After that, we will have to jump out of the system and to return to %s separately.
...
refuse_text = _([["As you wish, citizen. After all, one can not obligate people to do their duty..."]])


ham_title = _("Captain Hamfresser")
ham_text = _([["Hello, %s! You remember us?" Asks the captain, apparently unaware of the fact that his appearance is hard to forget. "We are ready to embark whenever you want, pilot! You'll just have to make space for %s of cargo (we have some equipment). Oh yes, and there is apparently a small change in the plan regarding the Captain Leblanc. She won't be able to join us. Talk to the Lieutenant, he will explain it better."]])

nikol_title = _("Sergeant Nikolov")
nikol_text = _([["Hello, citizen. I guess you would want to speak with the officers to have details about the plan. But tell me while you are here: there are many people in this room, but none of them is sitting at the tables next to ours. Do you know what is wrong with this part of the room?" You answer that the people probably feel unsafe sitting next to a group of dangerous-looking cyborgs like them, and she answers: "Meh, I don't think you're right. When I am in the Space Infantry refectory, there are dangerous-looking cyborgs all around and nobody feels unsafe. There should be something else..."]])

tronk_title = _("Private Tronk")
tronk_text = _([["I have ordered water and the waiter gave me that glass, with lots of bubbles. It does not look safe. Do you know what it is? I hope it's not alcohol..." As you tell him that it is simply sparkling water, and that it doesn't contain alcohol nor anything toxic, he gratefully answers: "Whoa, you really look like an expert in drinks! You know, I was asking because we cyborgs of class gamma can't drink alcohol. Each morning we have to take a special medication, the Spacemarine's Cocktail, to ensure that our organism supports our biological and cybernetic implants, and this cocktail is incompatible with any alcohol.
...
therus_title = _("Corporal Therus")
...
imp_no_text = _([["Mwell." Says the agent. "I guess you want to see for yourself that this is the only solution. If you're still alive when you're done, come back, we will be waiting for you."]])
...
pirate_text1 = _([[As you land, someone seems to be waiting for you on the spaceport. "Hello, colleague! Someone is in trouble with the authorities, out there. You seem to have had an argument with the Za'lek, and now the Imperials help them. I've seen blockades everywhere on the borders of Imperial space. Even the way to the secret jumps is impassable. It looks like the Empire wants to get you at all costs, but luckily enough, I have the solution. You probably already got a fake transponder, but they seem to have identified it, so what about receiving another one? I can sell you an authentic fake transponder, coming straight outta Skulls and Bones factory."
   This person is for sure a pirate who wants to take the opportunity to get a few credits. The idea is not bad as the imperial ships would not look for a ship with a Skulls and Bones fake transponder. So you ask him how many credits he wants. "%s" is the answer. "That sounds a great many, doesn't it? But maybe it's a suitable amount of money for your life and the success of whatever unscrupulous mission you're trying to carry out. Of course, you may not have such an amount right here, so I'll accept if you give your word to pay me at some point. Your word and your DNA signature as well, so that I can find you if you try to trick me."
...
tdie_text = _("You were supposed to disable that ship, not destroy it. How are you supposed to free anyone now?")

tesc_title = _("Mission Failed: target escaped")
tesc_text = _("You were supposed to disable that ship, not let it escape. How are you supposed to free anyone now?")
...
function convoyEnter()
   target = pilot.addFleet("Za'lek Sting", prevsys)[1]
   ...
   escort = {}
   escort[1] = pilot.addFleet("Za'lek Light Drone", prevsys)[1]
   escort[2] = pilot.addFleet("Za'lek Light Drone", prevsys)[1]
   escort[3] = pilot.addFleet("Za'lek Heavy Drone", prevsys)[1]
   --escort[4] = pilot.addFleet("Za'lek Heavy Drone", prevsys)[1]
...
function spawnDrones()
   pilot.addFleet("Za'lek Light Drone", lastPlanet)
   pilot.addFleet("Za'lek Light Drone", lastPlanet)
   pilot.addFleet("Za'lek Heavy Drone", lastPlanet)
...
function spawnZlkSquadron( pos, bloc )
   squad = {}
   squad[1]  = pilot.addFleet("Za'lek Mephisto", pos)[1]
   squad[2]  = pilot.addFleet("Za'lek Demon", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]
   squad[3]  = pilot.addFleet("Za'lek Demon", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]
   squad[4]  = pilot.addFleet("Za'lek Sting", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]
   squad[5]  = pilot.addFleet("Za'lek Sting", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]

   squad[6]  = pilot.addFleet("Za'lek Light Drone", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]
   squad[7]  = pilot.addFleet("Za'lek Light Drone", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]
   squad[8]  = pilot.addFleet("Za'lek Heavy Drone", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]
   squad[9]  = pilot.addFleet("Za'lek Heavy Drone", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]

   squad[10] = pilot.addFleet("Za'lek Bomber Drone", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]
   squad[11] = pilot.addFleet("Za'lek Bomber Drone", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]
   squad[12] = pilot.addFleet("Za'lek Bomber Drone", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]
   squad[13] = pilot.addFleet("Za'lek Bomber Drone", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]
...
function spawnEmpSquadron( pos, bloc )
   squad = {}
   squad[1]  = pilot.addFleet("Empire Hawking", pos)[1]
   squad[2]  = pilot.addFleet("Empire Pacifier", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]
   squad[3]  = pilot.addFleet("Empire Pacifier", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]
   squad[4]  = pilot.addFleet("Empire Admonisher", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]
   squad[5]  = pilot.addFleet("Empire Admonisher", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]

   squad[6]  = pilot.addFleet("Empire Shark", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]
   squad[7]  = pilot.addFleet("Empire Shark", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]
   squad[8]  = pilot.addFleet("Empire Lancelot", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]
   squad[9]  = pilot.addFleet("Empire Lancelot", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]

   squad[10] = pilot.addFleet("Empire Lancelot", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]
   squad[11] = pilot.addFleet("Empire Lancelot", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]
   squad[12] = pilot.addFleet("Empire Lancelot", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]
   squad[13] = pilot.addFleet("Empire Lancelot", pos + vec2.new(rnd.sigma()*300, rnd.sigma()*300))[1]
...
function spawnStrafer()
   strafer = pilot.addFleet("Trader Gawain")[1]
