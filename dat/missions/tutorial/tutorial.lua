...
function timer ()
   ...
   elseif mem.stage==4 and player.pos():dist(dest_planet:pos()) <= dest_planet_r then
      ...
      sai(fmt.f(_([["I will launch a combat practice drone off of {pnt} now for you to fight. Don't worry; our drone does not have any weapons and will not harm you. Target the drone by clicking on it or by pressing {targethostilekey}, then use your weapons, controlled with {primarykey} and {secondarykey}, to take out the drone!"]]),
         {pnt=dest_planet, targethostilekey=tut.getKey("target_hostile"), primarykey=tut.getKey("primary"), secondarykey=tut.getKey("secondary")}))
...
function pilot_death_timer ()
   ...
   sai(fmt.f(_([["I think we should try venturing outside this system. There are many systems in the galaxy; this one is but a tiny sliver of what can be found out there! Travelling through systems is accomplished with jump points. Like planets, you usually need to find these by exploring the area, talking to the locals, or buying maps. Once you have found a jump point, you can use it by #bright-clicking#0 on it or using the {jumpkey}."]]),{jumpkey=tut.getKey("jump")}))
