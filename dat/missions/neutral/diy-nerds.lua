...
function accept ()
   ...
   if not system_hasAtLeast(2, "land") then
      tk.msg(_("Young People"), text[17])
      misn.finish(false)
   end

   if not tk.yesno( _("A group of excited nerds"), string.format(text[1], destPlanet:name() )) then
      misn.finish(false)
   else
      if player.pilot():cargoFree() < 4 then
         tk.msg(_("Not enough cargo space"), text[16])
         misn.finish(false)
      end
      ...
      tk.msg(_("We have a deal!"), string.format(text[2], destPlanet:name()))
...
function nerds_land1()
   ...
   if cp == destPlanet then
      ...
      -- in time, right planet
         tk.msg(_("Happy nerds"), string.format(text[3], player.name(), srcPlanet:name() ))
      ...
      -- late, right planet
         tk.msg(_("Angry nerds"), string.format(text[4], srcPlanet:name() ))
         misn.finish(true)
      end
   else
      if not intime then
      -- late, not even the right planet
         tk.msg(_("Furious nerds"), text[5])
         misn.finish(true)
      end
   end
end
...
function nerds_land2()
   ...
   -- you pickup the nerds in time
      ...
      if nerdswon then
         tk.msg(_("Happy nerds"), string.format(text[8], srcPlanet:name()))
      else
         tk.msg(_("Sad nerds"), string.format(text[9], srcPlanet:name()))
      end
      cleanup()
      ...
      -- player has not enough free cargo space, give him last chance to make room
         tk.msg(_("Room for the box"), string.format(text[18], player.name()))
   ...
   -- you're late for the pickup
      tk.msg(_("No more nerds"), string.format(text[10], player.name()))
      cleanup()
      misn.finish(true)

   elseif not intime then
   -- you're late and far from the nerds
      tk.msg(_("You forgot the nerds"), string.format(text[11], srcPlanet:name()))
      cleanup()
      misn.finish(true)
   end
end
...
function nerds_fly2()
   if not hailed and time.get() > expiryDate then
      tk.msg(_("In-system communication"), string.format(text[6], srcPlanet:name()) )
   ...
   if hailed and intime and time.get() > expiryDate + time.create(0,2,0) then
      if not impatient then
         tk.msg(_("In-system communication"), string.format(text[7], srcPlanet:name()) )
         impatient = true
      end
...
function nerds_bar()
   ...
   hook.rm(lhook)
   if player.pilot():cargoFree() >= 4 then
      tk.msg(_("Departure"), text[19])
      nerds_return()
   else
      tk.msg(_("No room, no job"), text[20])
      misn.finish(true)
   end
end
...
function nerds_land3()
   ...
   if cp == srcPlanet then
      if nerdswon then
         tk.msg(_("The End"), string.format(text[12], player.name()))
         player.pay(30e3)
      else
         if not tk.yesno(_("Minor Complications"), text[13]) then
            tk.msg(_("So what?"), text[14])
         end
         tk.msg(_("The End"), text[15]:format( _(reward_outfit) ))
