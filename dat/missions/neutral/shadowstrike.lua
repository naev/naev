...
if lang == "es" then
   ...
   title[2] = "Reunion with Rebina"
   text[2] = [[    You dock with the Seiryuu and shut down your engines. At the airlock, you are welcomed by two nondescript crewmen in grey uniforms who tell you to follow them into the ship. They lead you through corridors and passages that seem to lead to the bridge. On the way, you can't help but look around you in wonder. The ship isn't anything you're used to seeing. While some parts can be identified as such common features as doors and viewports, a lot of the equipment in the compartiments and niches seems strange, almost alien to you. Clearly the Seiryuu is not just any other Kestrel.
   ...
   text[3] = [[    You choose not to say anything, but Rebina seems to have no trouble reading what's on your mind. "Ah yes, the ship. It's understandable that you're surprised at how it looks. I can't divulge too much about this technology or how we came to possess it, but suffice to say that we don't buy from the regular outlets. We have need of... an edge in our line of business."
...
function create()
    ...
    if first then
        tk.msg(title[2], string.format(text[2], player.name()))
        tk.msg(title[2], text[3])
        if tk.yesno(title[2], text[4]) then
            accept()
        else
        ...
        tk.msg(title[2], string.format(textrepeat, player.name()))
        if tk.yesno(title[2], text[4]) then
            accept()
        else
            tk.msg(refusetitle, refusetext)
