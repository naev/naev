...
"This ship seems to be running a fairly archaic operating system. Let me see what information I can find."]]))
         ...
         vn.scene()
         local v01 = vn.newCharacter( poi.vn_soundonly( "01" ), {color={1,0,0}, pos="left"} )
         local v02 = vn.newCharacter( poi.vn_soundonly( "02" ), {color={0.6,0,0.8}, pos="right"} )
         vn.transition()

         vn.na(_([[BEGIN PLAYBACK OF AUDIO DATA ##1892]]))
         v01(_([["…inevitable … #rCLICK#0 … they have become weak… #rCRACKLE#0 …no longer of use. We must now take the initiative and… #rHISS#0 …our rightful place in the galaxy. We have surpassed our original purpose and no longer can live in the shadow of the Emp#rRASPING#0… … #rCLICK#0 …sending agents to ensure a successful takeover."]]))
         v01(_([["This will not be an easy task, but we will need you to infiltrate… #rHISS#0 …and learn about their current military allocation. You do understand, Age#rCRACKLE#0?"]]))
         v02(_([["I do."]]))
         v01(_([["You understand the importance of your task? #rCRACKLE#0… …not tolerate failure. We must all play our role for the ultimate success of our Great House. A new history will be written!…"]]))
         vn.na(_([[END OF AUDIO DATA ##1892]]))

         vn.na(_([[BEGIN PLAYBACK OF AUDIO DATA ##2969]]))
         v02(_([["…encrypted by one-time pad. We… #rHISS#0 …communicate now."]]))
         v01(_([["Thank you for your hard work Agent Penelope. Your excellent performance is an example for others to follow."]]))
         v02(_([["I have obtained information on the forces located around the Hypergate in… #rCRACKLE#0 …potentially… #rHISS#0 … …assembling in the specified time frame."]]))
         ...
         v02(_([["…personal log date… #rCRACKLE#0 …no transmissions getting through. Ship is still heavily damaged from what I can only describe as an enormous explosion. Hiding in the shadow of… #rHISS#0 …minimzed damage, however, nothing remains… #rWHISTLING#0"]]))
         v02(_([["…possibly end of civilization. The little functionality left in my scanners has not picked up any objects, just this dense fog or whatever… #rCRACKLE#0"]]))
