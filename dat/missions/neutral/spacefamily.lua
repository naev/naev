...
function create ()
   ...
   tk.msg(_("Shipwrecked space family"), string.format(_([[Harrus is about to launch into another anecdote about his existence as a trader, but you manage to forestall him. You soon learn that his family's lifestyle has come to an abrupt change at the hands of a minor gang of pirates. Though the %s had some weaponry and shielding systems, the attackers were too much for a single cargo ship.
   ...
   tk.msg(_("Next stop"), string.format(directions[nextstop], destplanet:name(), destsys:name())) -- NPC telling you where to go
   misn.osdCreate(_("The Space Family"), {_("Take the space family to %s in the %s system"):format(destplanet:name(), destsys:name())})
...
function land()
   if planet.cur() == destplanet then -- We've arrived!
      if nextstop >= 3 then -- This is the last stop
         tk.msg(_("Rid of them at last"), string.format(_([[You land at your final stop in your quest to take the space family home, and not a moment too soon for both you and Harrus. Harrus stomps off your ship without so much as a greeting, his wife and children in tow, and you are just as happy to see them gone.
    ...
    Inside the box, you find a sum of credits and a note written in neat, feminine handwriting that says, "Sorry for the trouble."]]), destsys:name())) -- Final message
         player.pay(500e3)
...
function harrassme()
   tk.msg(_("Distractions on the bridge"), _([[You are going over a routine navigation check when Harrus enters your cabin unannounced. He seems to have recovered from his distressed state, and now radiates confidence.
