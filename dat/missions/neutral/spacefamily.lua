...
function create ()
    ...
    nextstop = 1
    targsys = {} -- Clear the array of target systems
    getsysatdistance(system.get(), 3, 3) -- Populate the array
...
-- Finds a random system with planets exactly n jumps away from system sys and places it in array targsys.
-- The first call to this function should always have n >= m
function getsysatdistance (sys, n, m)
    if n == 0 then -- This is a leaf call - perform checks and add if appropriate
        if sys:jumpDist() == m and haslandable(sys) then
            local seen = false
            for i, j in ipairs(targsys) do -- Check if the system is already in our array
              if j == sys then
                 seen = true
                 break
              end
...
function land()
    if planet.get() == destplanet then -- We've arrived!
        if nextstop == 3 then -- This is the last stop
            ...
            nextstop = nextstop + 1
            targsys = {} -- Clear the array of target systems
            getsysatdistance(system.get(), nextstop+1, nextstop+1) -- Populate the array
            if #targsys == 0 then targsys = {system.get("Apez")} end -- In case no systems were found.
            destsys = targsys[rnd.rnd(1, #targsys)] 
            destsysname = system.name(destsys)
            destplanet = getlandable(destsys) -- pick a landable planet in the destination system
