...
function create()
   ...
   local jumpreward = commodity.price(cargo)*1.5
   local distreward = math.log(100*commodity.price(cargo))/80
...
function land()
   if planet.cur() == destplanet then
      -- Semi-random message.
      lmisn.sfxMoney()
      vntk.msg( _("Delivery success!"), fmt.f(cargo_land[rnd.rnd(1, #cargo_land)], {cargo=_(cargo), credits=fmt.credits(reward)}) )
      player.pay(reward)
