...
function enter()
  ...
  if eavesdropped1 and eavesdropped2 and system.cur() == system.get(sysname2) and (not rescued) then
    kidnappers = pilot.addFleet("Trader Koala", planet.get("Zhiru"):pos() + vec2.new(-800,-800))[1]
...
function attackedkidnappers()
  ...
  if needpirates then
    bodyguard1 = pilot.addFleet("Pirate Hyena", vec2.new(800, 700))[1]
    bodyguard2 = pilot.addFleet("Pirate Hyena", vec2.new(-900, 600))[1]
    bodyguard3 = pilot.addFleet("Pirate Hyena", vec2.new(700, -500))[1]
    bodyguard1:control()
    bodyguard2:control()
    bodyguard3:control()
