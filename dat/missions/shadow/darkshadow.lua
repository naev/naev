...
function create()
   ...
   tk.msg(_("An urgent invitation"), _([[Suddenly, out of nowhere, one of the dormant panels in your cockpit springs to life. It shows you a face you've never seen before in your life, but you recognize the plain grey uniform as belonging to the Four Winds.
    ...
    The screen goes dead again. You decide to make a note of this in your log. Perhaps it would be a good idea to visit the Seiryuu once more, if only to find out how they got a private line to your ship!]]):format(player.name(), seirplanet:name(), seirsys:name()))
...
function accept2()
   ...
   misn.osdCreate(_("Dark Shadow"), osd_msg)
...
function seiryuuBoard()
   ...
   if stage == 1 then -- Briefing
      tk.msg(_("Disclosure"), _([[You make your way through the now familiar corridors of the Seiryuu. You barely notice the strange environment anymore. It seems unimportant compared to the strange events that surround your every encounter with these Four Winds.
    ...
    Captain Rebina stands up, a signal that this briefing is over. You are seen to your ship by a gray-uniformed crewman. You sit in your cockpit for a few hectoseconds before disengaging the docking clamp. What Captain Rebina has told you is a lot to take in. A shadowy organization that guides humanity behind the scenes? And parts of that organization going rogue? The road ahead could well be a bumpy one.]]):format(player.name(), jorekplanet1:name(), joreksys1:name(), jorekplanet1:name()))
      accept2()
      misn.markerRm(firstmarker)
      stage = 2
   elseif stage == 6 then -- Debriefing
      tk.msg(_("A safe return"), _([[You find yourself back on the Seiryuu, in the company of Jorek and the Four Winds informant. The informant is escorted deeper into the ship by grey-uniformed crew members, while Jorek takes you up to the bridge for a meeting with Captain Rebina.
    ...
    You return to your ship and undock from the Seiryuu. You reflect that you had to run for your life this time around, and by all accounts, things will only get worse with the Four Winds in the future. A lesser person might get nervous.]]):format(player.name(), player.name()))
      player.pay(1e6)
      seiryuu:control()
      seiryuu:hyperspace()
...
-- Board hook for Joe
function joeBoard()
   tk.msg(_("An extra passenger"), _([[You board the Four Winds vessel, and as soon as the airlock opens a nervous looking man enters your ship. He eyes you warily, but when he sees that Jorek is with you his tension fades.
    "Come on, %s," Jorek says. "Let's not waste any more time here. We got what we came for. Now let's give these damn vultures the slip, eh?"]]):format(player.name()))
...
function startAmbush()
   ...
   delay = delay + 5.0
   hook.timer(delay, "showMsg", {_("Ambush!"), _([[Suddenly, your long range sensors pick up a ship jumping in behind you. Jorek checks the telemetry beside you. Suddenly, his eyes go wide and he groans. The Four Winds informant turns pale.
...
-- NPC hook
function barman()
   tk.msg(_("A tip from the barman"), _([[You meet the barman's stare. He hesitates for a moment, then speaks up.
    ...
    You thank the barman, pay for your drink and prepare to head back to your ship, wondering whether your armaments will be enough to deal with whatever trouble Jorek is in.]]):format(player.name(), joreksys2:name(), jorekplanet2:name()))
...
-- NPC hook
function jorek()
   tk.msg(_("Still an unpleasant man"), _([["Well hello there %s," Jorek says when you approach his table. "It's about damn time you showed up. I've been wastin' credits on this awful swill for days now."
    ...
    "Yeah, right, about that. You know about the deal with the other branches getting too big for their britches? Good. Well, I've been lookin' into that, pokin' my nose into their business. Since I'm dealin' with my fellow Shadows here, I couldn't afford to give myself away. So that's that. But there's more."]]):format(player.name()))
