...
if lang == "es" then
    ...
    text[3] = [[    "The Four Winds are old, %s. Very old indeed. The movement dates back to old Earth, before the Space Age, even. We have been with human civilization throughout the ages, at first only in the Eastern nations, later establishing a foothold worldwide. Our purpose was to guide humanity, prevent it from making mistakes it could not afford to make. We were diplomats, scientists, journalists, politicians' spouses, sometimes even assassins. We used any means necessary to gather information and avert disaster, when we could.
...
function create()
    ...
    jorekplanet1, joreksys1 = planet.get("Edergast") -- TODO
    jorekplanet2, joreksys2 = planet.get("Edergast") -- TODO
...
function board()
    if stage == 1 then -- Briefing
        ...
        tk.msg(title[2], text[5]:format(player.name(), jorekplanet1, joreksys1, jorekplanet1))
        accept2()
        stage = 2
