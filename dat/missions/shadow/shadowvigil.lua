...
function enter()
    if system.cur() == misssys[1] and stage == 1 and missend == false then
        ...
        escorts = pilot.add("Shadowvigil Escorts", vec2.new(0, 0))
...
function jumpin()
    ...
    if stage == 4 then
        -- Spawn the diplomat.
        diplomat = pilot.add("Shadowvigil Diplomat", origin)[1]
    ...
    if stage >= 2 then
        ...
        -- Spawn the escorts.
        escorts = pilot.add("Shadowvigil Escorts", origin)
        for i, j in ipairs(escorts) do
            if not alive[i] then j:rm() end -- Dead escorts stay dead.
            if j:exists() then
                j:control()
        ...
        elseif system.cur() == misssys[3] then -- case join up with diplomat
            diplomat = pilot.add("Shadowvigil Diplomat", vec2.new(0, 0))[1]
            hook.pilot(diplomat, "death", "diplomatDeath")
            hook.pilot(diplomat, "jump", "diplomatJump")
            diplomat:control()
            diplomat:setInvincPlayer()
        ...
        elseif system.cur() == misssys[4] then -- case rendezvous with Dvaered diplomat
            ...
            dvaerplomat = pilot.add("Dvaered Vigilance", vec2.new(2000, 4000))[1]
            dvaerplomat:control()
            ...
            if jp2go <= 2 and jp2go > 0 then -- Encounter
                ambush = pilot.add(string.format("Shadowvigil Ambush %i", 3 - jp2go), vec2.new(0, 0), "baddie_norun")
                kills = 0
