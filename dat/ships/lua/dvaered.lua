...
function explode_init( p )
   ...
   mem.exploded = false
   mem.dtimer = math.min( 1 + math.sqrt( armour_max * shield_max ) / 750, 3 + math.pow( armour_max * shield_max, 0.4) / 600 )
   mem.timer = 0
...
function explode_update( p, dt )
   ...
   if mem.timer < 0 then
      ...
      explosion( pos, p:vel(), r*1.4, nil, tmerge( {silent=true}, exp_params(r) ) )
   end
