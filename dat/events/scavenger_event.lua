...
local chitchat_commander = {
	...
	_("I tried my luck at Minerva Station, but I couldn't stop looking at that chicken. Still gives me the creeps to think about it."),
	_("The best passengers are the youths. They know their place. Anyone past sixty feels as if they've survived three apocalypses. I guess they kind of did."),
...
local chitchat_rare = {
	...
	_("In this cold, vast emptiness I often reminisce of my adolescent years, the cycle we spent at Zhang Lu climbing the mountains. I fell in love with the nature. Then I fell in love with Jayne."),
...
local chitchat_dreamship = {
	...
	_("I figure you've probably spent a lot on me by now, but it was all worth it. I'm thankful for every day I get with my {dreamship}."),
...
local chitchat_affirm = {
	_("Affirimative."), _("Roger that."), _("Ten four."),
	_("I'm with you."), _("Okay."), _("I understand."),
	_("Aye commander!"), _("Gotcha."), _("Understood."),
	_("On you."), _("With you."),
	_("Yes Sir."), _("Aye Sir."),
	_("For the {dreamship}!"),
}
...
local function remember_ship ( me, ship, points )
	...
	print(fmt.f("{name} remembers {ship} is worth {points} points.", {name=me.name, ship=ship, points=points+current_score}))
	return
end
...
Escort paid tribute: {tribute} ]])
   return (approachtext .. "\n\n" .. fmt.f( credentials,  edata  ) .. "\n\n" ..
	   fmt.f( finances, {
	   credits=fmt.credits(edata.wallet), total=fmt.credits(edata.total_cost), debt=fmt.credits(edata.debt) , profits = fmt.credits(edata.total_profit), goal=edata.dreamship, tribute=fmt.credits(edata.tribute)
	   } ) )
end
...
local function _createReplacementShip ( persona , limit_ships )
	...
	local budget = math.floor(persona.wallet * (1 - persona.royalty) + (persona.deposit * persona.royalty) + persona.tribute * persona.experience)
...
--		persona.pilot:broadcast(fmt.f("I have boarded {boarded} times and made {profit} total.", { boarded=boarded, profit=persona.total_profit } ))
	...
	if persona.experience >= 1 then
		...
		if persona.pilot ~= nil then
			persona.pilot:broadcast(fmt.f("My budget was boosted to {budget} due to my experience level of {exp}", { budget=fmt.credits(budget), exp= persona.experience } ))
		end
	end
	...
	if persona.pilot ~= nil then
		persona.pilot:broadcast(fmt.f("My budget was {budget} and I chose a {ship} worth {deposit}. I have {credits} left and will cost up to {fee} to replace ({royalty_percent:.1f}%).", { budget = fmt.credits(budget), ship = persona.ship, deposit = fmt.credits(deposit), credits = fmt.credits(persona.pilot:credits()), fee = persona.replacement_text, royalty_percent=persona.royalty_percent }))
	end
...
local function create_pilot( fac )
	...
	local big_dreams = {
		"Kestrel",         "Hawking",
		"Pirate Kestrel",
		"Empire Hawking",  "Empire Rainmaker",
		"Dvaered Retribution",
	}
	
	local normal_dreams = {
		"Kestrel",         "Hawking",
		"Pirate Kestrel",  "Pirate Rhino"
	}
	...
	if pir.factionIsPirate( pf ) then
         ...
         { ship = "Pirate Phalanx",    royalty = 0.35 },
	      ...
	      dream_choices = {
		     "Pirate Starbridge", "Pirate Rhino",
			 "Pirate Admonisher", "Pirate Phalanx", "Pirate Ancestor",
		  }
	  if can_dream then
   ...
   elseif pf == faction.get("Empire") then
	  ...
	  dream_choices = {
		"Empire Pacifier", "Empire Admonisher"
	  }
		  portrait_arg = "Empire"
   ...
   elseif pf == faction.get("Dvaered") then
	  ...
	  dream_choices = {
		     "Dvaered Vigilance", "Koala",
			 "Dvaered Phalanx", "Dvaered Ancestor",
		  }
       if can_dream and fac:playerStanding() > 30 then
   ...
   elseif pf == faction.get("Thurion") then
	  ...
	  dream_choices = {
             "Thurion Ingenuity", "Thurion Scintillation", "Thurion Virtuosity", "Thurion Apprehension",
		  }
...
--		print(fmt.f("created a pilot named {name}", newpilot ) )
		local id = evt.npcAdd( "approachScavenger", _("Pilot for Hire"), newpilot.portrait, 
fmt.f(_([[This pilot seems to be looking for work.

Ship: {dreamship}
Initial Deposit: {deposit_text}
Cover fee: {royalty_percent:.1f}% of deposit]]), newpilot),
			9 - cmdr_fudge )
		npcs[id] = newpilot
	end
end
...
function approachScavenger( npc_id )
	...
	local rand_quotes = {
		...
		_("I heard you boarded and robbed an armed transport so fast that it was still drifting by the time you detached. Is that true?"),
	...
	local part1 = {
		_("I'll stick with you, mostly, and I'll do my part in disabling and boarding our enemies before we kiss them goodnight, permanently."),
    ...
    if not tk.yesno( _("Pilot Apprentice"), fmt.f(_(
...
Will you pay {credits} to kickstart my career as your apprentice?"]]
				 ...
				 part1=part1[rnd.rnd(1,#part1)], part2=part2[rnd.rnd(1,#part2)],
	...
	if pdata.commander then
		for _i, other in ipairs(mem.persons) do
			if other.commander then
				tk.msg( _("Unwelcome Animosity"), fmt.f(_([["I can't fly with {name}. We used to work together at Janus Station. All the talking about dreams of flying a {dreamship}, when clearly some old {ship} would be a much more appropriate career choice. Maybe someone else will tolerate that insufferable chatterbox."]]), other ) )
				return
			end
		end
	end
	...
	if #mem.persons >= max_scavengers then
		tk.msg( _("Too many affiliates"), _([["You look like you've got enough problems on your hands already. I think I'll stay out of your way for now."]]) )
		return
	end
    ...
    tk.msg( pick_one(title_choices), fmt.f(_([[You pay the young fool, who instantly scurries towards the shipyard.
    ...
    npcs[npc_id] = nil
    local id = evt.npcAdd(
         "approachHiredScav", pdata.name, pdata.portrait,
         _("This is a new pilot under your wing."), 8 )
    npcs[id] = pdata
    evt.save(true)

   shiplog.create( logidstr, _("Hired Escorts"), _("Hired Escorts") )
   shiplog.append( logidstr, fmt.f(_("You hired a {ship} ship named '{name}' for {deposit_text} at a {royalty_percent:.1f}% initial cover charge."), pdata ) )
end
...
function scav_boarding( plt, target, i )
	...
	if bounty > 0 then
		...
		shiplog.append( logidstr, fmt.f(_("'{name}' paid you {credits} from a total bounty of {total} ({ship})."), {name = mem.persons[i].name, ship = mem.persons[i].ship, credits = fmt.credits(your_share), total = fmt.credits(bounty)}) )
	else
		shiplog.append( logidstr, fmt.f(_("'{name}' gained {xp} experience boarding a creditless victim ({ship})."), {name = mem.persons[i].name, ship = mem.persons[i].ship, xp=exp_boost}) )
	...
	if rnd.rnd() < speak_chance then
		local info = {
			amount = fmt.credits(bounty), 			player = player.name(),
			flagship = player.ship(), 				dreamship = mem.persons[i].dreamship,
			flagshipt = player.pilot():ship():name(),
			your_share = fmt.credits(your_share), 	my_share = fmt.credits(bounty - your_share),
		}
...
--			persona.pilot:broadcast(fmt.f("I have boarded {boarded} times and made {profit} total.", { boarded=boarded, profit=persona.total_profit } ))
			...
			if profits > 0 then 
				...
				player.pay(your_share)
				shiplog.append( logidstr, fmt.f(_("'{name}' paid you {credits} from sold commodities worth {total} ({ship})."), {name = persona.name, ship = persona.ship, credits = fmt.credits(your_share), total = fmt.credits(profits)}) )
			end
		end
	end
   ...
   for i, edata in ipairs(mem.persons) do
      ...
      if edata.alive then
         ...
         local id = evt.npcAdd(
               "approachHiredScav", edata.name, edata.portrait,
               this_desc, 8 )
         npcs[id] = edata
         new_escorts[j] = edata
      end
...
function scav_attacked( p, attacker, _dmg, i )
   ...
   -- check if we should change states
	if rnd.rnd() > 0.98 and p:health() < 60 then
		if p:leader() then
			p:setLeader(nil)
			p:broadcast(fmt.f(leave_choices[rnd.rnd(1, #help_choices)], {name = player.name(), flagship=player.pilot():ship():name()}))
			mem.persons[i].last_sentiment = "beating"
		else
			p:setLeader(player.pilot())
			p:broadcast(fmt.f(help_choices[rnd.rnd(1, #help_choices)], {name = player.name(), flagship=player.pilot():ship():name()}))
		end
	end
end
...
Pilot credentials:]])
   ...
   local n, _s = tk.choice( "", getOfferText(approachtext, edata), upgradechoice, _("Abandon scavenger"), _("Do nothing") )
   if n == 1 and tk.yesno(
		"", fmt.f( upgradequery, { name = edata.name , credits = fmt.credits(edata.debt), upgrade_amount = fmt.credits(upgrade_amount) } )
		) then
		if edata.debt >= 1e6 then
			if player.pilot():credits() < edata.debt then
				-- not enough credits
				tk.msg( _("Not Enough Money"), _([["I can't fly on dreams and steam. Try again when you have enough money."]]) )
				return
			end
   ...
   elseif n == 2 and tk.yesno(
         "", fmt.f(
            _("Are you sure you want to get rid of {name}? This cannot be undone."),
	    edata ) ) then
		local k = #mem.persons
...
Pilot credentials:]])
   ...
   local n, _s = tk.choice( "", getOfferText(approachtext, edata), _(fmt.f("Tip Pilot {credits}", {credits=fmt.credits(tip_amount)})), _("Go scavange"), _("Call back") , _("Do nothing") )
   if n == 1 and tk.yesno(
         "", fmt.f(
            _("Are you sure you want to tip {name}? This will cost {price}."),
	    { name = edata.name , price = fmt.credits(tip_amount) } ) ) then
      shiplog.append( logidstr, fmt.f(_("You tipped '{name}' {credits} ({ship})."), { name = edata.name, ship = edata.ship, credits = fmt.credits(tip_amount) }) )
