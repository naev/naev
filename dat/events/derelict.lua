...
function goodevent()
   ...
   local fcts  = {}
   local HIT   = 10+rnd.rnd(0,10) -- between destroying a corvette / destroyer
   for f,i in pairs(sysfct) do
      -- Only insert factions that can still gain standing
      if faction.hitTest( f, HIT ) > 0 then
         table.insert( fcts, f )
      end
   end
   if #fcts > 0 then
      table.insert( goodevent_list, function ()
         ...
         faction.hit( rndfactRaw, HIT, nil, nil, true )
      end )
   end
   ...
   local maps = {
      ...
      ["Map: Empire-Soromid Trade Route"] = _("Empire-Soromid trade route"),
