...
local function getBarSituation( character_sheet )
	local bar_actions = {
		{
			["verb"] = pick_one({
				_("swirling"), _("sipping"), _("drinking"), _("enjoying"), _("nursing"), _("nursing on")
			}),
			["descriptor"] = pick_one({
				_("a"), _("a"), _("some"), _("some kind of")
			}),
			["adjective"] = pick_one({
				_("nice"), _("strange"), _("hot"), _("cold"), _("chilled"), _("colourful"), _("warm"),
			}),
			["object"] = pick_one({
				_("drink"), _("wine"), _("tea"), _("concoction"), _("elixir"), _("mixture of fluids"), _("beverage"),
			}),
		},
		{
			["verb"] = pick_one({
				_("thinking"), _("pondering"), _("wondering"), _("having feelings")
			}),
			["descriptor"] = pick_one({
				_("about some"), _("about a"), _("about that"),
			}),
			["adjective"] = pick_one({
				_("nice"), _("strange"), _("shady"), _("cold"), _("eerie"), _("colourful"), _("distracting"),
			}),
			["object"] = pick_one({
				_("drink"), _("person"), _("sculpture"), _("plant"), _("piece of machinery"), _("bartender"), _("smell"),
			}),
		},
		{
			["verb"] = pick_one({
				_("looking"), _("peering"), _("squinting"), _("eyeing")
			}),
			["descriptor"] = pick_one({
				_("at"), _("towards"), _("in the direction of"), _("vaguely towards")
			}),
			["adjective"] = pick_one({
				_("some"), _("a"), _("a mysterious"), _("an unsuspicious"), _("a suspicious looking"), _("an anonymous"), _("another"),
			}),
			["object"] = pick_one({
				_("drink"), _("patron"), _("person"), _("area"), _("stranger"), _("crowd"), _("group"),
			}),
		},
		{	-- an irregular one that adds anxiety to the crew
			["verb"] = pick_one({
				_("anxious"), _("desperate"), _("poised"), _("ready")
			}),
			["descriptor"] = _("to"),
			["adjective"] = pick_one({
				_("get back to"), _("return to"), _("retreat to"), _("abscond with")
			}),
			["object"] = pick_one({
				_("the ship"), _("a drink"), fmt.f(_("the {ship}"), {ship = player.pilot():name()}), _("some food")
			}),
		},
	}
...
local function create_memory (character, memory_type, params )
	...
	elseif memory_type == "friend" then
		...
		local choices = {
			...
			pick_one(actions) .. " talking a lot about things like the " .. pick_key(params.conversation.topics_liked) .. " or whatever.",
			pick_one(actions) .. " talking about the " .. pick_key(params.conversation.topics_liked) .. " or whatever.",
			pick_one(actions) .. " expressing concern when the conversation was focused on the " .. pick_key(params.conversation.topics_disliked) .. ".",
			pick_one(actions) .. " expressing concern when the conversation was focused on the " .. pick_key(params.conversation.topics_disliked) .. pick_one(when),
	...
	elseif memory_type == "animosity" then
		...
		local start = fmt.f(start, name)
		local insult = getInsultingProperNoun()
		local punctuation = pick_one({
			".",
			"!",
			"...",
			"!!",
		...
		if params.credits > 25e3 then
			...
			if params.credits > 187e3 then
				choices = {
					...
					_("My favourite ship to board is the {ship}. Well, I can have many favourites, but that's a top contender for sure."),
					_("That {target} never stood a chance."),
			...
			if params.credits > 50e3 then
				...
				if character.last_kill and character.last_kill == params.ship then
					...
					table.insert(choices, fmt.f(_("That {ship} was nothing but a {spacething} and its captain a {spacefool}."),
	...
	elseif memory_type == "hysteria" then
		...
		local choices = {
			...
			_("Sometimes I can't get the thought out of my head... It's just this thin piece of hull between us and nothingness for distances so vast you can't even realistically imagine."),
			...
			_("I used to dream of sailing on a pirate ship as a child. Unfortunately, we don't live in that kind of fairy tale world. I did my time on a pirate ship in space, but I realized that what I really wanted was to feel the salty sea air blow across my face and fling my hair into the air."),
		...
		local choices = {
			...
			fmt.f(_("I think there's an issue with our {outfit}, I'm going to go and check it out."), { outfit = pick_one(pso) } ),
			fmt.f(_("I was just doing a routine {outfit} inspection. I swear."), { outfit = pick_one(pso) } ),
			fmt.f(_("Before you go there, I don't want to hear about the {outfit} or the {system} story."), { outfit = pick_one(pso), system = "{system}" } ),
...
local function has_interest( character , interest )
	for topic, _phrases in pairs(character.conversation.topics_liked) do
		if topic == interest then return true end
	end
    ...
    if other and other.chatter * 1.5 > rnd.rnd() then
		...
		for interest, _phrases in pairs(other.conversation.topics_liked) do
			if string.find(spoken, interest) then
				print(fmt.f("detected {topic} from our list of topics {list}", {topic=interest, list=tostring(other.conversation.topics_liked)}))
		...
		local responses = {
			_("Whatever."), _("Yeah, okay."), fmt.f(_("Okay, {name}."), talker),
			fmt.f(_("Aright, {name}."), talker), _("Sure."), _("Oh, really?")
		} -- default responses
...
local function generateBackstory(cdata)
    ...
    elseif cdata.typetitle == "Engineer" then
        ...
        backstory.funfacts = {
			...
			fmt.f(_("My last captain named his ship the {shipname}. What an idiot. It had to go, it had to blow."), {shipname = getMadeUpName() }),
			fmt.f(_("One of my last captains almost renamed his ship to {shipname} in a drunken stupor... I made sure that couldn't happen."), {shipname = getMadeUpName() }),
            fmt.f(
        ...
        backstory.intent =
            pick_one(
            {
                ...
                _("I like to make conversation. Maybe I can join your crew and participate in all the fun and excitement that happens on your ship?"),
				...
				_("I'm looking for a ship for work. I'm not from around here and my last captain traded in his ship to pay off some debt."),
				...
				_("I inherited an interplanetary tea house franchise, but the relative ease of spacetravel completely destroyed the market. It only lasted me seven years. I wish it could have been eight."),
            }
        )
        backstory.origin =
            pick_one(
        ...
        backstory.funfacts = {
			...
			fmt.f(_("If anyone asks you where I'm from, tell them I'm from {origin}, see if they believe you."), backstory),
			fmt.f(_("Don't tell anyone that I'm from {origin}, not that they would believe you."), backstory),
			fmt.f(_("I used to work near Crylo and one day, {someone} caught a {fish} so big we were called down to transport it off-world on our Gaiwan."),
	...
	special_facts = {
			...
			fmt.f(_("I have a large scar on my leg. I got it in an altercation with a fiesty {person} on {place} after a slight misunderstanding."), { person = pick_one({ _("woman"), _("youngster"), _("acrobat"), _("man"), _("warrior")}), place = spob.get(faction.get("Dvaered")) } ),
			fmt.f(_("I have a hole in my leg. I got it after a minor misunderstanding with a {person} near {place}."), { person = pick_one({ _("drone"), _("robotic guard"), _("manual firearm"), _("fictional character"), _("badass")}), place = spob.get(faction.get("Za'lek")) } ),
			...
			_("I killed a man with my bare hands, it was intense. It was me or him. I was obviously outmatched, but I got lucky."),
			_("I was once a split second from getting blasted into bits by an armed guard near {place} when a masked stranger appeared out of nowhere and swapped out his plasma rifle with an umbrella! Yeah, I didn't believe it when it happened right in front of my eyes either."),
...
local function generateIntroduction(cdata)
    ...
    elseif cdata.chatter < 0.6 then -- I speak an average amount
		...
		if rnd.rnd(0, 1) == 0 then
			...
			approachtext = "{greeting} {intent} Oh, did I mention that my name is {name}? {reassurance}\n\n{funfact} {preprompt} {prompt}"
		end
    end
...
local function generateTopics()
	local topics = {
		...
		["luxury"] = {
			...
			_([[Here, smell this. It's called "the faithful friend", but it cost a lot of credits, almost as much as this ship.]]),
			_([[Here, smell this. It's called "the pacifier of violence", but it cost a lot of credits, almost as much as this ship.]]),
			fmt.f(_("You should try this new virtual viewport simulator, it lets you pretend you're on a {ship}."), { ship = getRandomShip() } ),
		...
		["ship"] = {
			...
			fmt.f(_("This is my favourite ship. The {name} of {made_up} was alright but nothing like this."), { made_up = getMadeUpName(), name = pilotname.human() } ),
			fmt.f(_("This is such a nice ship. The {made_up} of {name} was decent but nothing like this."), { made_up = getMadeUpName(), name = pilotname.human() } ),
			...
			fmt.f(
				_("I used to serve on a {ship}. I think I've told you about it, right?"),
				{ship = getRandomShip()}
			),
			fmt.f(
				_(
					"I once saw a comet shaped like a {ship} near {place}. It was pretty cool."
				),
				{ship = getRandomShip(), place = spob.get(true)}
			),
			fmt.f(_("I've heard from {name} the new {ship} is going to be even sleeker than the current model."), { name = pilotname.human(), ship = getRandomShip() } ),
			fmt.f(_("I've heard from my old friend {name} the next {ship}, code-name {made_up} is going to have a special compartment for {thing}s."), { ship = getRandomShip(), name = pilotname.generic(), made_up = getMadeUpName(), thing = getRandomThing() }),
			...
			fmt.f(_("Did you read that article about the 7 {ship} features no captain knows about?"), {ship = getRandomShip()}),
		},
		["visit"] = {
			fmt.f(
			...
			fmt.f(
				...
				{place = spob.get(faction.get("Empire")), name = pilotname.human(), relative = pick_one({ _("aunt"), _("grandmother"),_("uncle"), _("grandfather"), _("councellor")})}
			),
			fmt.f(
		...
		["affair"]  = {
			fmt.f(_("I too have had my fair share of affairs. One day I might tell you the story of {name}."), { name = pilotname.human() } ),
			...
			fmt.f(_("I got into a mighty struggle with {name} of {made_up} before I joined this ship."), { made_up = getMadeUpName(), name = pilotname.human() } ),
			...
			fmt.f(_("... I'm not sure that's appropriate but whatever. I'm sure {captain} doesn't mind. Oh, hey {captain}!"), { captain = player:name() } )
		},
		["travel"] = {
			fmt.f(
			...
			fmt.f(
				_(
					"I had an affair with a warrior from {place}. I wonder what fearsome {name} is up to these days."
				),
			...
			fmt.f(
				_(
					"I went to {place} just to check it out. I haven't had the urge to go since."
				),
				{place = spob.get(faction.get("Soromid"))}
			),
		},
	   ["violence"] = {
			...
			fmt.f(_("I could slay you with a defective {thing} in a {thong} battle."), { thing = getMadeUpName(), thong = getMadeUpName() } ),
			...
			_("I know we're not supposed to have blades like these in the spaceport but this baby never leaves my side, I take it everywhere."),
			...
			fmt.f(_("Check out this {made_up}. I only paid {amount} for this killer. How many credits is that per ship destroyed?"), { made_up = getMadeUpName() , amount = fmt.credits(rnd.rnd(35e3, 725e3)) }),
			_("I would never kill my friends, but I could break your leg on a whim."),
			fmt.f(
				_(
					"All the violence and lawlessness on {place} excites me."
				),
				{place = spob.get(faction.get("Dvaered"))}
			),
			fmt.f(
				_(
					"I got into a scuffle on {place} some cycles back. That's where I got the small scar."
				),
				{place = spob.get(faction.get("Dvaered"))}
			),
			fmt.f(
				_(
					"We all make mistakes. I once killed a man on {place} only to find out I got the wrong guy."
				),
				{place = spob.get(faction.get("Empire"))}
			),
			fmt.f(
		...
		["science"] = {
			...
			fmt.f(
				_("I wonder what the big deal about {place} is, there's no mystery."),
				{place = spob.get(true)}
			),
			fmt.f(
				_(
			...
			fmt.f(_("I've heard from {name} the new Admonisher is going to be even sleeker than the current model."), { name = pilotname.human() } ),
			fmt.f(_("I've heard from {name} the next {ship}, code-name {made_up} is going to have a secret unscannable cargo compartment."), { ship = getRandomShip(), name = pilotname.generic(), made_up = getMadeUpName()}),
			fmt.f(_("The new '{made_up}' {thing} is supposedly the bee's knees."), { made_up = getMadeUpName(), thing = getRandomThing() }),
			fmt.f(_("I've heard that an updated {thing} is going to be even sleeker than the current model."), {thing = getRandomThing()}),
			fmt.f(_("I've heard that the new {thing} is going to be even sleeker than the current model."), {thing = getMadeUpName()}),
			...
			fmt.f(_("Did you hear that the {thing} now only costs {amount}?"), {thing = getMadeUpName(), amount=fmt.credits(rnd.rnd(20e3,50e3-1))}),
			fmt.f(_("I've tried that new {thing} and I have to say, I'm amazed."), {thing = getMadeUpName()}),
			fmt.f(_("I don't think those paid science publications are any good. I put all my faith in {thing}."), {thing = getMadeUpName()}),
			fmt.f(_("{thing} luxury friendship bracelets are practically being given away for just a single credit if you adopt an abandoned illegal pet iguana and fill out some required forms. The only downside is that we'd have to travel to {place} to go get it."), {thing = getMadeUpName(), place = spob.get(faction.get("Empire"))}),
		}
	}
...
local function createGenericCrewmate()
   ...
   local pf = spob.cur():faction()
	...
	character.deposit =
		math.ceil(75e3 * character.satisfaction * character.xp + character.chatter * rnd.rnd() * 8e3)
	character.salary = 0
	...
	character.conversation.bar_actions = {
		{
			["verb"] = pick_one({
				_("swirling"), _("sipping"), _("drinking"), _("enjoying"), _("nursing"), _("nursing on"), _("chugging")
			}),
			["descriptor"] = pick_one({
				_("a"), _("another"), _("some"), _("some kind of")
			}),
			["adjective"] = pick_one({
				_("cheerful"), _("bizarre"), _("steaming"), _("iced"), _("chilled"), _("extravagant"), _("foaming"),
			}),
			["object"] = pick_one({
				_("drink"), _("wine"), _("tea"), _("concoction"), _("elixir"), _("mixture of fluids"), _("beverage"), _("beer"), _("coffee"), _("spirit")
			}),
		},
	}
...
local function createEscortCompanion()
	...
	crewmate.conversation = {
		...
		["satisfied"] = {
			...
			fmt.f(_("I recently acquired a sample of {made_up}'s latest youth serum. Would anyone care to try some?"), { made_up = getMadeUpName() } ),
		...
		_("I'm a bit tired."),
		_("I could use some rest."),
		_("You know I need my wealthy planets, don't let me be the black sheep of the bunch that's bringing everyone down."),
		},
		["bar_actions"] = {
			{
				["verb"] = pick_one({
					_("seducing"), _("talking to"), _("gesturing at"), _("giggling at")
				}),
				["descriptor"] = pick_one({
					_("some"), _("a"), _("a"),
				}),
				["adjective"] = pick_one({
					_("nice looking"), _("strange"), _("shady"), _("handsome"), _("dark"), _("mysterious"), _("preoccupied"), _("unknown"),
				}),
				["object"] = pick_one({
					_("stranger"), _("person"), _("man"), _("woman"), _("interloper"), _("dock worker"), _("crew member"),
				}),
			}
		},
		-- special things I know how to say
		...
		["topics_liked"] = {
			...
			["travel"] = {
				...
				fmt.f(
					_(
						"I fell in love with a pirate from {place}. I wonder what {article}'s up to these days."
					),
				...
				fmt.f(
					_(
						"I went to {place} just to check it out. I don't recommend it."
					),
					{place = spob.get(faction.get("Soromid"))}
				),
			},
	...
	table.insert(crewmate.manager.lines.satisfied, _("With a captain like you, it's no wonder we're all so happy. There's nothing to worry about."))
	table.insert(crewmate.manager.lines.satisfied, _("You shouldn't be having any problems with this crew. Everyone seems to be perfectly happy."))
	table.insert(crewmate.manager.lines.satisfied, _("I think the crew could use a break on a nice luxurious world, but for now there's nothing to worry about."))
	table.insert(crewmate.manager.lines.satisfied, _("Don't worry about the crew, there's nothing wrong that a spa won't fix."))
	...
	table.insert(crewmate.manager.lines.unsatisfied, _("I wouldn't worry about the crew, but they are getting kind of tense."))
	...
	table.insert(crewmate.manager.lines.unsatisfied, _("With a captain like you, it's no wonder we're usually all so happy. I'm sure things will get better."))
	table.insert(crewmate.manager.lines.unsatisfied, _("With a captain like you, it's a wonder the situation is so dire."))
...
function land()
    ...
    for i, edata in ipairs(mem.companions) do
        ...
        sss = math.floor(10 * (sss - (sss / 64))) / 10 + 0.01 * rnd.threesigma()
		...
		local elsewhere_chance = (#mem.companions - edata.satisfaction) / (edata.xp + #mem.companions + math.abs(edata.satisfaction))
		if edata.manager then elsewhere_chance = 0 end -- don't let managers go on holiday
		if mem.summon_crew or rnd.rnd() > elsewhere_chance or edata.manager then
			...
			local description = fmt.f(_("This is {firstname} {name}, one of your crew. It seems that {article_subject} is "), edata) .. doing
    ...
    local total_paid = 0
    for _item, paid in pairs(paid) do
...
function period_fatigue()
	...
	for _i, companion in ipairs(mem.companions) do
			...
			if rnd.rnd() < companion.chatter then
				...
				if companion.conversation.sentiment == last_sentiment then
					hook.timer(
						7 + rnd.rnd(3, 25),
						"say_specific",
						{me = companion, message = last_sentiment}
					)
		...
		elseif rnd.threesigma() > 2.5 and not hysteria then
			...
			local ramblings = _("This voyage is driving me crazy.")
...
local function crewmate_barConversation(edata, npc_id)
	...
	if not edata.firstname then edata.firstname = "Mysterious" end -- generic dreadful nick, but better than "Gendrick"
...
function escort_landing(speaker)
    ...
    local good_tags = {
        ["medical"] = {
			"worlds", "facilities", "planets", "institutions", "complexes"
		},
        ["military"] = {
			"worlds", "facilities", "outposts", "organizations", "complexes"
		},
	...
	local neutral_choices = {
		...
		fmt.f(_("I didn't want to ask in front of that {made_up}, but do you think it's real?"), {made_up = getMadeUpName()}),
	}
    -- check good tags
    for tag, thing_choices in pairs(good_tags) do
        if tags[tag] then
            ...
            if rnd.rnd() > 0.33 then
                relevant_message =
                    fmt.f(pick_one(good_choices), {tag = tag, place = thing, made_up = getMadeUpName()})
            end
        end
    end
    ...
    for tag, thing in pairs(neutral_tags) do
        if tags[tag] then
            ...
            if not relevant_message and rnd.rnd() > 0.67 then
                relevant_message = fmt.f(pick_one(neutral_choices), {tag = tag, place = thing, made_up = getMadeUpName()})
            end
        end
    end
...
function player_boarding_c4(target, speaker)
    if target:hostile() then
		...
		if target:ship():size() >= 5 and rnd.rnd() < speaker.chatter then
			create_memory(speaker, "violence", {
				target = target:name(),
				ship = target:ship(),
