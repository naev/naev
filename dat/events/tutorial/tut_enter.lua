...
"It looks like we entered part of the volatile nebula. The instability here causes heavy damage to any ships that enter. If our shield regeneration surpasses the volatility damage, we should be fine. However, ff the volatility gets any stronger, it could be fatal to the {shipname}. Going deeper into the nebula could prove to be a very risky endeavour."]]), {shipname=player.ship(), ainame=tut.ainame()} ) )
