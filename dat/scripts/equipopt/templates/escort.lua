    ...
    for i, edata in ipairs(mem.persons) do
        ...
        if edata.alive then
			...
			if edata.ship:nameRaw() ~= edata.dreamship then
				this_desc = this_desc .. _("\nThis pilot is currently flying a {ship} at a cover fee of {replacement_text}, which will be added to the pilot's personal account of ") .. fmt.f("{credits}.", { credits = fmt.credits(edata.wallet) } )
			end
    ...
    if
        n == 1 and
            tk.yesno(
                "",
                fmt.f(
                    _("An effective commendation will cost {price} and increase {rank}s favor with the {ship}. Are you sure you want to commend the efforts of {rank} {name} of {ship}?"),
