...
local function print_debug( p, st, ss, outfit_list, params, constraints, energygoal, emod, mmod, nebu_row, budget_row )
   ...
   if nebu_row then
      ...
      print(string.format(_("Shield Regen: %.3f [%.3f > %.3f (%.1f)]"), stn.shield_regen, constraints[nebu_row] or 0, nebu_vol*(1-ss.nebu_absorb_shield)-st.shield_regen, 1))
   end
end
...
function optimize.optimize( p, cores, outfit_list, params )
   ...
   if nebu_vol > 0 then
      rows = rows+1
      nebu_row = rows
      nebu_dmg = nebu_vol*(1-ss.nebu_absorb_shield)
   ...
   for i,s in ipairs(slots) do
      for j,o in ipairs(s.outfits) do
         ...
         if nebu_vol > 0 then
            ...
            table.insert( ja, c )
            table.insert( ar, stats.shield_regen + nebu_vol*stats.nebu_absorb_shield )
         end
         -- Limit constraint
         if stats.limit then
