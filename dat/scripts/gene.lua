...
local function create_group_rec( grp, node, x )
   x = x or 0
   if node._g then return end
   node._g= true
   node.x = x
   table.insert( grp, node )
   grp.x  = math.min( grp.x, node.x )
   grp.y  = math.min( grp.y, node.y )
   grp.x2 = math.max( grp.x, node.x )
   grp.y2 = math.max( grp.y, node.y )
   for i,c in ipairs(node.conflics or {}) do
      create_group_rec( grp, skills[c], x+1 )
   end
   if node.required_by then
      create_group_rec( grp, node.required_by, x )
   end
   for i,r in ipairs(node.requires or {}) do
      create_group_rec( grp, skills[r], x )
   end
   return grp
end
...
for k,s in pairs(skills) do
   if not s._g then
      local grp = create_group_rec( {x=math.huge,y=math.huge,x2=0,y2=0}, s )
      grp.w = grp.x2-grp.x
      grp.h = grp.y2-grp.y
   ...
   for x=0,20 do
      if fits_location( x, g.y, g.w, g.h ) then
         ...
         for i,s in ipairs(g) do
            ...
            print( string.format( "%s : %d x %d", s.name, px, py ) )
