--[[
   The new "slimplus" GUI
   based on famous "slim" GUI
--]]
...
local ta_pane_w, ta_pane_x, ta_pane_y, ta_pnt_pane_x, ta_pnt_pane_y
...
function create()
   ...
   x_ammo = 600+16
   y_ammo = screen_h-28-16

   -- how many bars will fit in row (minus 1 for Planet pane)
   cnt_ammo = math.floor( (screen_w - (600+16+16)) / 134 ) - 1
   if cnt_ammo < 1 then
      cnt_ammo = 1
   end
   ...
   -- Set OSD
   gui.osdInit( 23, screen_h - 63 - 10, 150, 500 )
...
--]]
   ...
   slot_start_x = 175 --screen_w/2 - #aset/2 * slot_w
end
...
function render_cooldown( percent, _seconds )
   ...
   gfx.print(false, _("Cooling down..."), cooldown_frame_x,
         cooldown_bg_y + 8, cols.txt_bar, cooldown_frame_w, true )
end


local function render_bar( data, value, txt, txtcol, size, col, bgc )
   local offsets, l_bg_bar
   if size then
...
function render( dt, dt_mod )
   ...
   -- Effects
   local ex, ey = screen_w-48, screen_h-28-16-( (math.floor(#wset / cnt_ammo) + 1) * 28)
   ...
   for num, weapon in ipairs(wset) do
      ...
      if weapon.left then -- Truncate names for readability.
         ...
         render_ammoBar( "ammo", x_ammo + ( ( num - 1 ) % cnt_ammo ) * 134, y_ammo - math.floor( ( num - 1 ) / cnt_ammo ) * 28, values, txt, col)
      else
         col = cols.txt_bar
         ...
         render_ammoBar( "heat", x_ammo + ( ( num - 1 ) % cnt_ammo ) * 134, y_ammo - math.floor( ( num - 1 ) / cnt_ammo ) * 28, values, txt, col )
      end
   end
...
--      local y = y_ammo - #wset * 28 - 15
      ...
      local x = x_ammo + ( #wset % cnt_ammo ) * 134
      local y = y_ammo - math.floor( #wset / cnt_ammo ) * 28 + height
      ...
      gfx.renderRect( x, y-height, width, height, col)
      gfx.renderTex( bgs.bar_weapon, x, y-height )
      gfx.print( true, _("Set formation"), x, y + 8 - height, cols.txt_bar, width, true )
   end

   -- Active outfits
   if #aset > 0 then
      for i=1,#aset do
         local slot_x = slot_start_x + (i-1) * (slot_w+3) + 3

         -- Background for this slot.
         gfx.renderRect( slot_x, slot_y, slot_w, slot_h, cols.slot_bg )
