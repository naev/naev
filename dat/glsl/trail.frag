...
subroutine vec4 trail_func_prototype( vec4 colour, vec2 pos_tex, vec2 dim );
...
vec4 trail_default( vec4 colour, vec2 pos_tex, vec2 pos_px )
{
   float m;
...
/* Pulsating motion. */
TRAIL_FUNC_PROTOTYPE
vec4 trail_pulse( vec4 colour, vec2 pos_tex, vec2 pos_px )
{
   float m, v;
   ...
   // Modulate width
   colour.a *= smoothbeam( pos_tex.y, 2.0*m );

   v = smoothstep( 0.0, 0.5, 1.0-pos_tex.x );
   colour.a *=  0.8 + 0.2 * mix( 1.0, sin( 2.0*M_PI * (0.06 * pos_px.x + dt * 3.0) ), v );
...
TRAIL_FUNC_PROTOTYPE
vec4 trail_wave( vec4 colour, vec2 pos_tex, vec2 pos_px )
{
   float m, p, y;
...
TRAIL_FUNC_PROTOTYPE
vec4 trail_flame( vec4 colour, vec2 pos_tex, vec2 pos_px )
{
   float m, p, y;
...
TRAIL_FUNC_PROTOTYPE
vec4 trail_nebula( vec4 colour, vec2 pos_tex, vec2 pos_px )
{
...
TRAIL_FUNC_PROTOTYPE
vec4 trail_arc( vec4 colour, vec2 pos_tex, vec2 pos_px )
{
   float m, p, v, s;
   vec2 ncoord;
...
/* Bubbly effect. */
TRAIL_FUNC_PROTOTYPE
vec4 trail_bubbles( vec4 colour, vec2 pos_tex, vec2 pos_px )
{
